...
namespace KLS.Common
{
    public static class Utilities
    {
        ...
        public static dynamic ExtractMetadata(string base64String)
        {
            if (string.IsNullOrWhiteSpace(base64String))
                throw new ArgumentException("Base64 string cannot be null or empty.");

            // Check if the string contains metadata
            if (!base64String.Contains(",") || !base64String.Contains(";"))
                throw new FormatException("The Base64 string does not contain valid metadata.");

            // Extract the metadata part
            var metadataPart = base64String.Substring(0, base64String.IndexOf(","));

            // Extract ContentType
            var contentType = metadataPart.Split(':')[1].Split(';')[0];

            // Infer FileName (optional)
            var extension = contentType switch
            {
                ...
                "application/pdf" => ".pdf",
                "text/plain" => ".txt",
                _ => string.Empty // Unknown type
            };

            var fileName = $"file{extension}"; // Default file name

            return new { ContentType = contentType, FileName = fileName };
        }

        public static List<IFormFile> ConvertBase64StringsToIFormFiles(List<string> filesData)
        {
            ...
            foreach (var fileData in filesData)
            {
                ...
                var meta = ExtractMetadata(base64String);
                ...
                // Create a MemoryStream for the file bytes
