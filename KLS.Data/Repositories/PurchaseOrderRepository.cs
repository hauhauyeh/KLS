...
namespace KLS.Data.Repositories
{
    public class PurchaseOrderRepository : KLSRepository<PurchaseOrder>, IPurchaseOrderRepository
    {
        ...
        public IQueryable<POList> GetPagedList(POListReq purchaseOrderReq)
        {
            ...
            return DbContext.POList.FromSqlRaw("[dbo].[PurchaseOrder_GetAllList] @Pageno,@Pagesize,@Search,@StartDate,@EndDate,@VendorId,@EmpId,@Filterby,@Id,@SortField,@SortOrder,@IsCount,@DropShipSalesCustomerId,@TotalCount OUTPUT", param);
        }
        ...
        public int Count(POListReq purchaseOrderReq)
        {
            ...
            DbContext.Database.ExecuteSqlRaw("[dbo].[PurchaseOrder_GetAllList] @Pageno,@Pagesize,@Search,@StartDate,@EndDate,@VendorId,@EmpId,@Filterby,@Id,@SortField,@SortOrder,@IsCount,@DropShipSalesCustomerId,@TotalCount OUTPUT", param);
        ...
        private static object[] BuildParam(POListReq poReq)
        {
            object[] param = {
                ...
                poReq.DropShipSalesCustomerId.HasValue ? new SqlParameter("@DropShipSalesCustomerId", poReq.DropShipSalesCustomerId) : new SqlParameter("@DropShipSalesCustomerId", DBNull.Value),
        ...
        public int Checkout(POCheckoutReq checkoutReq)
        {
            ...
            DbContext.Database.ExecuteSqlRaw("[PurchaseOrder_Insert] @PurchaseId,@PayeeId,@PurchaseDate,@ArrivalDate,@Notes,@EmpId,@NewPOId OUTPUT,@FactorPO", PurchaseIdParam, PayeeIdParam, PurchaseDateParam, ArrivalDateParam, NotesParam, EmpIdParam, NewPOIdParam, FactorPOParam);
        ...
        public void CopyToBill(POCopyToBillReq copyToBillReq)
        {
            ...
            var OrderModeParam = (!string.IsNullOrWhiteSpace(copyToBillReq.OrderMode))
                ? new SqlParameter("@OrderMode", copyToBillReq.OrderMode)
                : new SqlParameter("@OrderMode", DBNull.Value);

            DbContext.Database.ExecuteSqlRaw("[PurchaseOrder_CopyToBill] @PurchaseId,@ItemsJson,@EmpId,@ShipmentIds,@OrderMode",
                POIdParam, PayeeIdParam, EmpIdParam, ShipmentIdsParam, OrderModeParam);
        }
    }
}
