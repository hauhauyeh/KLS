...
namespace KLS.Data.Repositories
{
    public class OpenBalanceRepository : IOpenBalanceRepository
    {
        ...
        public List<OpenBalanceExcelRow> Preview(string filePath, OpenBalanceSection section, out DateTime? downloadedAt)
        {
            ...
            var rows = DbContext.OpenBalanceExcelRow
                .FromSqlRaw("[dbo].[OpenBalance_ImportPreview] @FilePath,@Section,@DownloadedAt OUTPUT",
                    filePathParam, sectionParam, downloadedAtParam)
                .ToList();
        ...
        public List<OpenBalanceExcelRow> GetSectionRows(OpenBalanceSection section)
        {
            var sectionParam = new SqlParameter("@Section", OpenBalanceSectionInfo.Get(section).Token);

            return DbContext.OpenBalanceExcelRow
                .FromSqlRaw("[dbo].[OpenBalance_GetRows] @Section", sectionParam)
                .ToList();
        }
    }
}
