    ...
    public class PurchaseService : BaseService, IPurchaseService
    {
        ...
        public PurchaseList? UpdatePartially(int purchaseId)
        {
            ...
            if (purchase?.IsDropShip == true && purchase.StageId == 6)
                throw new ArgumentException("Use the restricted drop-ship Bill update.");
        ...
        public PurchaseList? DropShipRestrictedUpdate(int purchaseId)
        {
            var purchase = GetById(purchaseId)
                ?? throw new ArgumentException("Drop-ship Bill not found.");

            if (!purchase.IsDropShip || purchase.StageId != 6)
                throw new ArgumentException("This operation supports billed drop-ship purchases only.");

            Uow.Purchases.DropShipBillRestrictedUpdate(purchaseId);
            return GetListById(purchaseId);
        }
