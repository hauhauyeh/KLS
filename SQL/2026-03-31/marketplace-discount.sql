...
ALTER PROCEDURE [dbo].[MarketPlaceOrder_Convert]
    @MarketAccountId INT,
    @OrderDate DATE,
    @EmpId INT,
...
BEGIN
    ...
    WHERE mo.MarketAccountId = @MarketAccountId
      ...
      AND mo.ImportedToErp = 0 AND mo.OrderStatus = 'shipped'
      AND EXISTS (
    ...
    -- Step 5: Insert item lines into TempSales (SalesId = 0 for new order)
    ...
    WHERE moi.UnitPrice >= 0
    ORDER BY qo.ExternalOrderId

    -- Step 5b: Insert discount account lines for orders with discounts
    DECLARE @IDG_AccountId INT;
    SELECT @IDG_AccountId = AccountId FROM Account WHERE AccountCode = '@IDG';

    INSERT INTO TempSales (EmpId, SalesId, PayeeId, LineType, AccountId,
        OrdQty, ShipQty, BillQty, UnitPrice, Notes, ChangeStatus, IsTaxable)
    SELECT
        @EmpId, 0, @PayeeId, 'A', @IDG_AccountId,
        -1, -1, -1,
        disc.DiscountTotal,
        'MKT#' + qo.ExternalOrderId + ' DISCOUNT',
        'I', 0
    FROM #QualifyingOrders qo
    CROSS APPLY (
        SELECT
            COALESCE(
                NULLIF(SUM(CASE WHEN moi.UnitPrice < 0 THEN ABS(ROUND(moi.Qty * moi.UnitPrice, 2)) ELSE 0 END), 0),
                NULLIF(SUM(moi.DiscountAmount), 0),
                NULLIF(MAX(mo.DiscountAmount), 0),
                0
            ) AS DiscountTotal
        FROM MarketOrderItem moi
        INNER JOIN MarketOrder mo ON mo.MarketOrderId = qo.MarketOrderId
        WHERE moi.MarketOrderId = qo.MarketOrderId
    ) disc
    WHERE disc.DiscountTotal <> 0;
