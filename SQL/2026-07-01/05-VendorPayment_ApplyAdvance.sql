...
-- Fix: Add IsVoid=0 filter to exclude voided advance payments from auto-apply
-- Bug 6: After VoidCheck fix (VPD amounts preserved), voided advances would have
--         non-zero VPD amounts, preventing them from being re-applied after un-void

CREATE OR ALTER PROCEDURE [dbo].[VendorPayment_ApplyAdvance]
    @PurchaseId INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        -------------------------------------------------------------------
        -- Build all available vendor advances
        -- Rule:
        --   1. Previous vendor credits first
        --   2. Current PO-linked advance after that
        -------------------------------------------------------------------
        ...
        WHERE vp.PayeeId = @PayeeId
          ...
          AND vp.IsVoid = 0   -- FIX: exclude voided advance payments
        ...
        ORDER BY
            --CASE WHEN CurPO.PaymentDetailId IS NULL THEN 0 ELSE 1 END,   -- previous credit first
