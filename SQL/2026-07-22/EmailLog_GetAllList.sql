-- EmailLog_GetAllList.sql
-- Phase 5 of the EmailLog system-wide audit plan.
-- Rewrites the list SP to:
--   * return the new audit columns,
--   * fix the search bug (was gated by ISNUMERIC, so non-numeric name/email
--     search did nothing) and extend search to document number / subject / type,
--   * add Category / EmailType / DeliveryStatus filters,
--   * parameterize all user input via sp_executesql (no string concatenation),
--   * whitelist the sort column.
--
-- Backward-compatible deploy: the three new params are appended AFTER
-- @TotalCount OUTPUT with NULL defaults, so the current repository call (10
-- positional params) still binds correctly. Extra result columns are ignored by
-- the current EmailLogDTO. Deploy this BEFORE the API (Phase 5B) ships.
...
PRINT 'EmailLog_GetAllList updated successfully.';
GO
