...
CREATE OR ALTER PROCEDURE [dbo].[Report_PackingList_1B_Test] --[Report_PackingList] '02/05/2026','E',null
    @ShipDate   DATE,
...
BEGIN
    ...
    SELECT
        ...
        sd.Notes,
        s.ShipRoute,
        -- 2026-05-01 ET: derive the effective load route inline so the report
        -- does not call dbo.Fn_Calc_EffectiveLoadRoute once per grouped row.
        CASE
    ...
    SELECT
        ...
        s.ShipRoute,
        -- 2026-05-01 ET: derive the effective load route inline so the report
        -- does not call dbo.Fn_Calc_EffectiveLoadRoute once per detail row.
        CASE
            WHEN NULLIF(LTRIM(RTRIM(s.ShipRoute)), '') IS NULL THEN NULL
            WHEN ISNULL(s.IsLoadSeparate, 0) = 1 AND s.RouteOrder IS NOT NULL
                THEN s.ShipRoute + CONVERT(VARCHAR(12), s.RouteOrder)
    ...
    BEGIN
        ...
        SELECT
            ...
            p.PayeeName,
            s.ShipRoute,
            -- 2026-05-01 ET: derive the effective load route inline so the
            -- customer marker follows the same route bucket as item rows.
            CASE
                WHEN NULLIF(LTRIM(RTRIM(s.ShipRoute)), '') IS NULL THEN NULL
                WHEN ISNULL(s.IsLoadSeparate, 0) = 1 AND s.RouteOrder IS NOT NULL
                    THEN s.ShipRoute + CONVERT(VARCHAR(12), s.RouteOrder)
