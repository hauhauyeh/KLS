...
BEGIN
    ...
    -- Slice 3: factory progress can move drop-ship PO to stage 2 or 3 before customer receipt.
    -- Receipt is still allowed only before bill conversion / later PO stages.
    IF @PurchaseStageId NOT IN (1, 2, 3)
    BEGIN
    ...
    -- 2026-08-22 DS-SHIPQTY-STAGE-AUTHORITY: share the canonical PO lock with
    -- factory progress, stage synchronization, and the Order-stage full editor.
    SET @LockResource =
    ...
    BEGIN TRY
        ...
        -- Revalidate the linked document state after both canonical locks are held.
        SET @PurchaseStageId = NULL;
        SELECT @PurchaseStageId = p.StageId
        FROM dbo.Purchase p
        WHERE p.PurchaseId = @PurchaseId
          AND ISNULL(p.IsDropShip, 0) = 1
          AND p.DropShipSalesId = @SalesId;

        IF ISNULL(@PurchaseStageId, -1) NOT IN (1, 2, 3)
        BEGIN
            RAISERROR('Purchase is not in an open drop-ship PO stage. Drop-ship receipt can only be updated before Bill conversion.', 16, 1);
        END

        SET @SalesStageId = NULL;
        SELECT
            @SalesStageId = s.StageId,
            @TaxRate = s.TaxPercent,
            @PaymentApplied = ISNULL(s.PaymentApplied, 0),
        ...
        IF ISNULL(@SalesStageId, -1) NOT IN (0, 1, 2)
        BEGIN
            RAISERROR('Drop-ship receipt can only be updated before customer receipt is confirmed.', 16, 1);
        END
    ...
    -- Section 7: sync the linked DS PO pre-bill stage after customer receipt.
    -- The helper owns DS factory-progress stage and does not move received Sales backward.
    EXEC [DropShipment_SyncSalesTransitFromPO] @PurchaseId;

    -- Do not run Sales_CalcTotal here. It would run final FIFO allocation after Stage 3.
