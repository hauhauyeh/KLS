/*
    ...
    Single change from the 2026-05-23 live baseline: ORDER BY for the default
    (no @SortField) path switched back from ArrivalDate -> EnterDate.

    Affects:
      - Recent filter (no search, no sort field): ORDER BY p.EnterDate DESC, p.PurchaseNumber DESC
      - Default with search:                       ORDER BY p.EnterDate DESC, p.PurchaseNumber DESC

    Why (history of this line):
      Original: ORDER BY p.EnterDate DESC
      2026-05-23: switched to ArrivalDate DESC + PurchaseNumber DESC tiebreaker.
                  Rationale was that VendorPayment_InsertPayNow can backdate
                  EnterDate via admin Excel import, sinking bulk-imported rows
                  mid-list under the Recent view.
      2026-05-27 (this change): user reports the ArrivalDate sort does not
                  match their day-to-day mental model of "recent" - they expect
                  the most recently entered bills at the top. The Excel-import
                  backdating case is rare enough that the trade-off favors
                  EnterDate. The PurchaseNumber DESC tiebreaker added on 5/23
                  is preserved (cheap, unambiguous secondary key).
