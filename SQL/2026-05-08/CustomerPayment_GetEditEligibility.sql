...
CREATE OR ALTER PROCEDURE [dbo].[CustomerPayment_GetEditEligibility]
...
BEGIN
    ...
    /*
        ...
        - if the payment already has an issued refund, block edit
    ...
    */
    ...
    -- First confirm whether the payment exists and capture its payment date,
    -- payment number, and lock state.
    -- Both date and payment number are needed later because the current rule treats "future" as:
    --
    -- - later PaymentDate
    ...
    -- 2026-05-08:
    -- The old deposited-payment full read-only block is retired.
    -- Deposited payments now reopen in partial-lock mode through the application layer:
    --
    -- - customer, date, method, and amount stay protected
    -- - reapply, discounts, notes, and reference remain editable
    --
    -- The backend save guard now enforces those protected fields directly, so
    -- deposit lock is no longer a full edit-eligibility blocker here.
    --
    -- -- If the payment has already been locked by deposit flow, block edit immediately.
    -- -- Deposited payments must open read-only even if they have no downstream dependency.
    -- IF @IsLocked = 1
    -- BEGIN
    --     SELECT
    --         CAST(0 AS BIT) AS CanEdit,
    --         CAST(1 AS BIT) AS IsReadOnly,
    --         @LockedReason AS Reason;
    --     RETURN;
    -- END;
