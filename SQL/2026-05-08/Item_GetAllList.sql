-- Item_GetAllList rev 2026-05-08 (I-only forward-removal)
-- Removes the @ShowDeleted parameter and the IsDeleted projection added in
-- the 2026-05-07 round. Keeps @ShowInactive (the I toggle on the page list)
-- and unconditionally filters i.IsDeleted = 0 -- deleted items are always
-- hidden from the list now that the D toggle is gone.
--
-- Also drops i.IsDeleted from the SELECT and the IsDeleted column from
-- #itmtbl; the DTO ItemList no longer carries IsDeleted, so projecting it
-- would throw at EF materialization.
--
-- The previous round's (@ShowInactive + @ShowDeleted) parameter pair and
-- 2-bit branch are preserved as -- comments above the new code.
--
-- Workflow: 5-step SQL workflow per CLAUDE.md. This file is the
-- workspace-root scratch; baseline is at
-- KLS\SQL\2026-05-08\Item_GetAllList_live_baseline.sql.
