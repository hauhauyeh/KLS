...
-- EXEC dbo.DropShipment_InsertSalesAndPO @SalesId=0, @PayeeId=1, @VendorPayeeId=2, @ShipDate='2026-07-21', @ShipRoute=NULL, @Instruction='Web', @EmpId=1, @PurchaseDate=NULL, @NewSalesId=0, @NewPurchaseId=0, @FactorPO='FPO-1', @CustPONumber='CPO-1', @Sour
ceSalesId=NULL;
...
BEGIN
    ...
    BEGIN
        ...
        -- 2026-08-02 DROPSHIP-EDIT-SHIPQTY-GUARD:

        -- Drop-ship edit rebuilds the linked PO detail. Validate the linked PO before

        -- entering the transaction, then re-check factory ShipQty under the purchase

        -- app lock before any SalesDetail/PurchaseDetail delete.
        ...
        BEGIN
            ...
            IF @ExistingPurchaseStageId NOT IN (1, 2)

            BEGIN

                RAISERROR('Linked drop-ship PO is no longer editable.', 16, 1);
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            SET @PurchaseLockResource = 'DropShip_UpdateShipQty_' + CONVERT(NVARCHAR(20), @ExistingPurchaseId);
            ...
            IF EXISTS (
                ...
                WHERE PurchaseId = @ExistingPurchaseId
                  ...
                  AND ISNULL(ShipQty, 0) <> 0

            )

            BEGIN

                RAISERROR('Linked drop-ship PO already has factory ship qty. Edit is no longer allowed.', 16, 1);
