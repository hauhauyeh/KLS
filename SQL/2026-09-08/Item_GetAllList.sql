...
GO
CREATE OR ALTER PROCEDURE [dbo].[Item_GetAllList] -- DECLARE @TotalCount INT; EXEC dbo.Item_GetAllList @Pageno=1,@Pagesize=50,@Search=N'1500''',@StartDate=NULL,@EndDate=NULL,@VendorId=NULL,@Container=NULL,@CategoryId=NULL,@Filterby=NULL,@Id=NULL,@SortField=NULL,@SortOrder=NULL,@IsCount=0,@TotalCount=@TotalCount OUTPUT,@ShowInactive=0,@ShowNonInventory=0
(
    @Pageno        INT,
    @Pagesize      INT,
    @Search        NVARCHAR(200),
...
BEGIN
    ...
    (
        ...
        BaseP1 DECIMAL(18,2),
        -- 2026-08-26: MGP product-list pricing columns. Always projected so the
        -- shared API shape stays stable; frontend decides whether to display them.
        NCost1 DECIMAL(18,4),
        NCost2 DECIMAL(18,4),
        TCost1 DECIMAL(18,4),
        TCost2 DECIMAL(18,4),
    ...
    ELSE
        SET @Qry = '
            SELECT
                ...
                bu.P1,
                -- 2026-08-26: expose existing item cost fields for the MGP-only
                -- pricing layout. Position MUST match #itmtbl.
                i.NCost1,
                i.NCost2,
                i.TCost1,
                i.TCost2,
    ...
    SET @Qry += '
        ...
        WHERE i.ItemType IN (''Inventory'', ''NonInventory'')
          AND i.IsDeleted = 0
          -- 2026-09-08: N toggle is authoritative; search and exact id no longer bypass it.
          -- Old: AND (@Id IS NOT NULL OR @SearchTerm IS NOT NULL OR @ShowNonInventory = 1 OR i.ItemType = ''Inventory'')
    ...
    BEGIN
        ...
        -- ------------------------------------------------------------------
        -- 2026-09-08: I toggle is authoritative during search too.
        -- Old: IF @ShowInactive = 0 AND @SearchTerm IS NULL SET @Qry += ' AND i.Inactive = 0';
        IF @ShowInactive = 0 SET @Qry += ' AND i.Inactive = 0';
    END;
