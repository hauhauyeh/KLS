...
BEGIN
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            WHERE pd.PurchaseId = @PurchaseId
              AND pd.ItemId IS NOT NULL; -- PHYSICAL_LANDED_COST: drop-ship physical item landed cost clears to @COGS.
    ...
    BEGIN TRY
        ...
        DECLARE @NonInvLandedTotal DECIMAL(18,2);
        DECLARE @InvClearCrDe DECIMAL(18,2);
        DECLARE @CommissionCrDe DECIMAL(18,2);

        SELECT @InvClearTotal = SUM(x.LandedCost),
        @CommissionTotal = SUM(CASE WHEN x.ItemType = 'Inventory' THEN x.ImportCommission ELSE 0 END),
        @NonInvLandedTotal = SUM(CASE WHEN x.ItemType <> 'Inventory' THEN x.LandedCost ELSE 0 END)
        FROM #TxImpact x;

        IF ABS(ISNULL(@NonInvLandedTotal, 0)) >= 0.01 AND @CogsAccountId IS NULL
            RETURN;
        ...
        -- PHYSICAL_LANDED_COST: normal non-inventory physical lines cannot carry landed cost in @INV.
        -- Clear their allocated landed cost from @INVC to @COGS at line grain.
        DELETE tgt
        FROM dbo.TransactionJournalDetail tgt
        WHERE tgt.TxId = @TxId
          AND tgt.AccountId = @CogsAccountId
          AND tgt.Notes = 'Non-Inventory Landed Cost'
          AND NOT EXISTS
          (
              SELECT 1
              FROM #TxImpact src
              WHERE src.PurchaseDetailId = tgt.SourceDetailId
                AND src.ItemType <> 'Inventory'
                AND ABS(ISNULL(src.LandedCost, 0)) >= 0.01
          );
        ...
        (
            SELECT
                @TxId AS TxId,
                x.PurchaseDetailId AS SourceDetailId,
                @CogsAccountId AS AccountId,
                @PayeeId AS PayeeId,
                x.ItemId,
                CAST(ROUND(ISNULL(x.LandedCost, 0), 2) AS DECIMAL(18,2)) AS Amount,
                crde.CrDeAmount
            FROM #TxImpact x
            CROSS APPLY dbo.Fn_CrDeAmount(CONVERT(NVARCHAR(20), @CogsAccountId), CAST(ROUND(ISNULL(x.LandedCost, 0), 2) AS DECIMAL(18,2))) crde
            WHERE x.ItemType <> 'Inventory'
              AND ABS(ISNULL(x.LandedCost, 0)) >= 0.01
        ) AS src
        ON  tgt.TxId = src.TxId
        AND tgt.AccountId = src.AccountId
        AND tgt.SourceDetailId = src.SourceDetailId
        ...
        WHEN MATCHED THEN
            UPDATE SET
                ...
                tgt.Amount = src.Amount,
                tgt.CrDeAmount = src.CrDeAmount,
                tgt.Notes = 'Non-Inventory Landed Cost'
        WHEN NOT MATCHED THEN
            INSERT (
                TxId, SourceDetailId, AccountId, PayeeId, ItemId,
                Qty, Price, BillQty, FactorToBase,
            ...
            VALUES (
                src.TxId, src.SourceDetailId, src.AccountId, src.PayeeId, src.ItemId,
                0, 0, 0, 1,
                src.Amount, src.CrDeAmount, 'Non-Inventory Landed Cost'
            );
        ...
        IF EXISTS (
            ...
            WHERE ItemType = 'Inventory'
              AND (LandedCost > 0 OR ImportCommission > 0)
        )
            SET @HasAllocation = 1;
