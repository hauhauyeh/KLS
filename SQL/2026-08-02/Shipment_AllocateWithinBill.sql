...
-- down to their own bill's physical item lines, per ShipmentCharge.LineBasis.
...
BEGIN
    ...
    -- Eligible physical item lines for this bill, with every basis weight (live Shipment_Allocation expressions).
    ...
    JOIN   dbo.Item i ON pd.ItemId = i.ItemId
    WHERE  pd.PurchaseId = @PurchaseId
      AND  pd.ItemId IS NOT NULL; -- PHYSICAL_LANDED_COST: include inventory and non-inventory physical items.
    ...
    -- Legacy NO_LANDED_COST rows remain inert for backward compatibility.
