...
--     VolumeOk/WeightOk = every eligible physical item line has the raw attribute AND the bill total > 0
...
-- Bills with no eligible physical item lines still return a row (all flags 0, totals 0).
-- =====================================================================
...
BEGIN
    ...
    ;WITH L AS (
        -- eligible physical item lines per bill, with each basis weight (live Shipment_Allocation expressions)
        SELECT
        ...
        JOIN   dbo.Item i ON i.ItemId = pd.ItemId -- PHYSICAL_LANDED_COST: include inventory and non-inventory physical items.
