...
BEGIN
    ...
    /*
        2026-05-03 Phase 5 ownership change:
        The old sync logic rebuilt TruckNumber, Driver, and TruckRouteOrder from Sales.
        Assign Truck now owns those route-level assignment fields in SalesRoute, so this
        procedure must no longer overwrite them from Sales.

        Old logic retained for reviewer comparison:
    ...
    */
    ...
    /*
        Existing SalesRoute rows now preserve their saved assignment fields exactly as
        stored. This sync procedure only maintains route shells for the selected date.
    */
    SET @UpdatedCount = 0;
