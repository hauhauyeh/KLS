...
-- First round for this SP — snapshot the existing version as _prev (idempotent).
IF EXISTS (SELECT 1 FROM sys.procedures WHERE name = 'Payee_UpdateAging')
   AND NOT EXISTS (SELECT 1 FROM sys.procedures WHERE name = 'Payee_UpdateAging_prev')
    EXEC sp_rename 'Payee_UpdateAging', 'Payee_UpdateAging_prev';
GO
