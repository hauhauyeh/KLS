...
GO
-- 2026-06-26: Sales Commission 3 -- NEW report (Sales Commission 2 kept untouched for comparison).
-- Same commission BASIS as v2 (invoices fully paid, AmountDue=0, settled by a CustomerPayment in
-- the window -- commission earned on collection), but cost/revenue come from the REAL transaction
-- journal instead of SalesDetail.FIFOCost:
--   * Cost    = @COGS                                  (recognized cost of goods sold)
--   * Revenue = (@ISALE + @ICREDIT) - Sales.DiscountApplied
--               @ISALE   = inventory sales income (verified: ALL item sales post here, 0 exceptions)
--               @ICREDIT = customer credit (verified: ONLY 'Sales Credit Memo' posts here -- returns)
-- Why minus DiscountApplied: @ISALE is GROSS (the invoice posts full revenue; the discount is a
-- PAYMENT-TIME discount recorded on Sales.DiscountApplied + CustomerPaymentDetail, not in the
-- invoice journal). v2 nets it out of commissionable revenue, so v3 does too -> revenue == v2.
-- FIFOCost (v2's cost) is NULL/0 across KLS_2026 (weighted-avg DB; FIFO gated by ITEM_USE_EXPIRYDATE,
-- off here), so v2's cost = 0 -> margin = 100% -> every rep hits the top commission tier. v3 fixes it.
-- Invoice journal link: TransactionJournal.SourceDocNumber = Sales.SalesNumber, across the 'Sales'
-- and 'Sales Credit Memo' doc types.
--
-- 2026-06-27 PERF: the original CTE form ran 3-4 minutes for one month. Cause was NOT a missing
-- index (the journal scan is a trivial ~1046 pages) -- it was the CTE+IN+EXISTS shape making the
-- optimizer re-evaluate the paid-invoice set repeatedly. Rewritten to materialize the paid invoices
-- into an indexed #temp once, then JOIN the journal to it -> ~1.4s for a month. No new index needed.
DROP PROCEDURE IF EXISTS [dbo].[Report_SalesCommission3]
GO
