...
GO
-- 2026-08-29 IsPricePending (plan-reprice-open-orders-v1 slice 6): one column added to the dynamic SELECT (inside the @Qry string, alias s = Sales).
--   SalesList is produced only here (Sales_DropShipRestrictedUpdate re-execs this SP). Baseline: Sales_GetAllList_live_baseline.sql
...
BEGIN
    ...
    ELSE
        ...
        SELECT
            ...
            s.RouteOrder,
            s.IsLocked,
            s.IsPricePending,   -- 2026-08-29 IsPricePending (plan-reprice-open-orders-v1 slice 6)
            s.IsLoadSeparate,
            s.TruckNumber,
