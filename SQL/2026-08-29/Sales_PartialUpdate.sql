...
GO
-- 2026-08-29 IsManualPrice pass-through (plan-reprice-open-orders-v1 slice 2) (staging table, load, loop SELECT, 'I' INSERT, 'U' UPDATE)
--   + IsPricePending cleared on human save before COMMIT. Baseline: Sales_PartialUpdate_live_baseline.sql
-- FIX_ICREDIT_AMOUNT_SIGN + FIX_ACCTLINE_AMOUNT_SIGN 2026-07-16: journal rows follow the model
-- (Amount = account inc/dec, set FIRST; then CrDeAmount = Fn_Adjust_CrDeAmount(Amount)).
--   - Item revenue rows (@ISALE/@ICREDIT): the value's sign chooses the account, so Amount =
--     magnitude (@ABSAmount). 3 spots: insert NonInventory, insert Inventory, update item row.
--   - Account-line rows (LineType='A'): fixed user account per row + document-signed @ExtTotal, so
--     Amount = @AcctAmount = account inc/dec (flip @ExtTotal for debit-normal accounts). 2 spots:
--     insert + update. CrDeAmount is UNCHANGED (= @ExtTotal, the balancing value); only Amount
--     becomes the inc/dec. Precheck 2026-07-16 confirmed all account-line CrDe = ExtTotal.
-- See baseline: KLS/SQL/2026-07-16/Sales_PartialUpdate_live_baseline.sql
-- ============================================================================
...
BEGIN
	...
	DECLARE
        ...
        @IsSystemManaged BIT,
        @IsManualPrice BIT,          -- 2026-08-29 IsManualPrice pass-through (plan-reprice-open-orders-v1 slice 2)
        @DisplaySort INT,
        @TempSalesId_Current INT,
		@ABSAmount DECIMAL(18,2),
		@IsAccountDebit BIT,
		@AcctAmount DECIMAL(18,2);
	...
	DECLARE @TempSaleTable TABLE (
		...
		[IsManualPrice] [bit] NOT NULL,   -- 2026-08-29 IsManualPrice pass-through (plan-reprice-open-orders-v1 slice 2)
	...
	INSERT INTO @TempSaleTable
		([LineId],[LineType],[ItemId],[AccountId],[ItemUnitId],[Unit],
		 ...
		 [CartLineType],[IsSystemManaged],[DisplaySort],[IsManualPrice],
	...
	BEGIN
        ...
        BEGIN
            ...
            IF @LineType='A'
		    BEGIN
                -- 2026-07-16 FIX_ACCTLINE_AMOUNT_SIGN: Amount = account inc/dec (flip document-signed @ExtTotal
                -- for debit-normal accounts), then derive CrDeAmount. Matches Sales_Insert account-line model.
                SET @IsAccountDebit = NULL;   -- reset first so a missing account is caught, not a stale prior-row value
                SELECT @IsAccountDebit = IsAccountDebit FROM dbo.Account WHERE AccountId = @AccountId;
                IF @IsAccountDebit IS NULL
                    RAISERROR('Sales_PartialUpdate: account-line AccountId has no Account row; cannot derive CrDeAmount.', 16, 1);
                SET @AcctAmount = CASE WHEN @IsAccountDebit = 1 THEN -@ExtTotal ELSE @ExtTotal END;
                EXEC Fn_Adjust_CrDeAmount @AccountId,@AcctAmount,@CrDeAmount OUTPUT
            ...
            BEGIN
                ...
                BEGIN
				    ...
				    VALUES
					    ...
					    ,@UnitPrice
					    ,@ExtTotal
                        -- 2026-07-16 FIX_ICREDIT_AMOUNT_SIGN: Amount = magnitude (@ABSAmount), the value fed to Fn_Adjust_CrDeAmount.
                        -- OLD: ,@ExtTotal -- amount
                        ,@ABSAmount -- amount
						,@CrDeAmount
						,@SalesDetailId)
        ...
        BEGIN
			...
			SET
				...
				IsSystemManaged   = @IsSystemManaged,
				DisplaySort       = @DisplaySort
				,IsManualPrice    = @IsManualPrice   -- 2026-08-29 IsManualPrice pass-through (plan-reprice-open-orders-v1 slice 2)
			FROM SalesDetail
			WHERE SalesDetailId = @SalesDetailId
			...
			BEGIN
				...
				Price=@UnitPrice,
				BillQty=@ExtTotal,
				-- 2026-07-16 FIX_ICREDIT_AMOUNT_SIGN: Amount = magnitude (@ABSAmount).
				-- OLD: Amount=@ExtTotal,
				Amount=@ABSAmount,
				CrDeAmount=@CrDeAmount,
				ItemId=@ItemId,
	...
	-- 2026-08-29 (plan-reprice-open-orders-v1, Q4): any human save clears the price-pending
	-- flag; the system reprice (EmpId = -1) manages the flag itself in Sales_RepriceOpenOrders.
	IF @EmpId <> -1
		UPDATE dbo.Sales SET IsPricePending = 0 WHERE SalesId = @SalesId AND IsPricePending = 1;
