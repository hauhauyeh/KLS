...
BEGIN
    ...
    SET @InvoiceDate = ISNULL(@InvoiceDate, CAST(GETDATE() AS DATE));
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            UPDATE PurchaseDetail SET
                ...
                ReceiveQty = OrdQty1,
                FinalQty = OrdQty1,
                BillExtTotal = ROUND(ISNULL(OrdQty0, 0) * ISNULL(BillPrice, 0), 2),
                FinalExtTotal = ROUND(ISNULL(OrdQty1, 0) * ISNULL(FinalPrice, 0), 2),
        ...
        SELECT @PurFinalTotal = ISNULL(SUM(ROUND(ISNULL(FinalQty, 0) * ISNULL(FinalPrice, 0), 2)), 0)
        FROM PurchaseDetail
        WHERE PurchaseId = @PurchaseId;
        ...
        CROSS APPLY (
            SELECT CASE
                WHEN ROUND(ISNULL(pd.FinalQty, 0) * ISNULL(pd.FinalPrice, 0), 2) = 0 THEN 0
        ...
        UPDATE Purchase SET
            ...
            VendorDocNumber = ISNULL(@VendorDocNumber, VendorDocNumber),
            VendorTotal = @PurFinalTotal,
            PurchaseTotal = @PurFinalTotal,
            AmountDue = @PurFinalTotal,
    ...
    EXEC [Sales_CalcTotal] @SalesId, @SalesTotalOut OUTPUT;
    EXEC [Payee_UpdateAging] @VendorPayeeId, 0;
END
GO
