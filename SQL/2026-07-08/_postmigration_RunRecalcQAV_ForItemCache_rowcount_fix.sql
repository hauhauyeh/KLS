...
CREATE OR ALTER PROCEDURE [dbo].[_postmigration_RunRecalcQAV_ForItemCache]
...
BEGIN
    ...
    */
    ...
    -- 2026-07-08 fix: @@ROWCOUNT was read again below, AFTER the SELECT (which resets it
    -- to 1) -> the message wrongly reported "1". Capture the UPDATE's rowcount immediately.
    DECLARE @UpdatedItemRows INT = @@ROWCOUNT;

    -- OLD: SELECT @@ROWCOUNT AS UpdatedItemRowCount;
    SELECT @UpdatedItemRows AS UpdatedItemRowCount;

    -- OLD: 'RecalcQAV run finished. Updated item cache rows = ' + CONVERT(VARCHAR(20), @@ROWCOUNT) + '.';
    SET @FinishMessage =
        'RecalcQAV run finished. Updated item cache rows = ' + CONVERT(VARCHAR(20), @UpdatedItemRows) + '.';
