...
--   Completeness guard & safe fallback.
--   BEFORE: a charge's basis fired if "ISNULL(st.TotalXxx,0) > 0" — i.e. ANY line in the shipment
--           had the attribute. So a shipment where only 4 of 1000 lines carried ItemVolume still
--           allocated BY_VOLUME, the 996 volume-less lines got LineVolume = 0 (absorbed zero
--           freight), and the whole freight charge piled onto those 4 lines. Wrong.
--   AFTER:  a dimensional method (BY_DUTY/BY_TARIFF/BY_VOLUME/BY_WEIGHT) is honored ONLY when
--           EVERY participating line in the SHIPMENT has the attribute (all-or-nothing). If any
--           line is missing it, the whole charge falls back to BY_VALUE (the only basis present on
--           every line) and is stamped 'BY_VALUE_FALLBACK' for audit.
--   Completeness is tested by ATTRIBUTE PRESENCE (ItemVolume>0, CaseWeight>0,
--   CustomDutyRate+TariffPercent>0) — NOT value/qty-scaled — so a legitimate free line (price 0)
--   does not falsely trip the fallback. BY_QUANTITY/BY_VALUE need no guard (always present).
--   Money is unchanged where data is complete; on KLS_2026 today (ItemVolume empty everywhere)
--   this is a behavioral no-op — every BY_VOLUME charge already fell back to BY_VALUE.
-- =============================================================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- intentionally left WITHOUT the all-or-nothing completeness guard for now (Plan 1 §9 open
