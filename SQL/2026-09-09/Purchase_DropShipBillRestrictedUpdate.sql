...
-- 2026-09-09: Exclude allocation-owned landed-cost @COGS rows from primary item-cost validation/update.
        ...
        BEGIN
            ...
            IF (SELECT COUNT(*) FROM dbo.TransactionJournalDetail tjd
                ...
                WHERE tjd.TxId=@TxId AND tjd.AccountId=@CogsAccountId AND pd.PurchaseId=@PurchaseId
                  AND ISNULL(tjd.Notes,'') NOT IN ('Drop-Ship Landed Cost','Non-Inventory Landed Cost'))
               <> @ExpectedCogsRows
