...
GO
-- 2026-09-09: Gate @COGS/@INV account validation by changed inventory bomb rows so non-inventory clients can save Bomb Sales.
-- FIX_ICREDIT_AMOUNT_SIGN 2026-07-16 follow-up (2026-07-17): journal rows follow the model
...
--     magnitude (ABS(lu.ExtTotal), the same base fed to Fn_CrDeAmount). 2 spots: Section 10
--     UPDATE + Section 11 INSERT. Was storing signed lu.ExtTotal (negative for @ICREDIT rows).
--   - @AR (Section 21) / @FSTP (Sections 22-23) rows already follow the model -- untouched.
--   Missed by the 2026-07-16 sales-group fix set (Sales_Insert + Sales_PartialUpdate +
--   CustomerPayment_Insert); this SP is the same U-branch posting logic as Sales_PartialUpdate
--   and must deploy with that group. Data cleanup: TxDetail_FixAmountSign_292_310.sql.
-- See baseline: KLS/SQL/2026-07-17/TempBombSales_Save_live_baseline.sql
-- ============================================================================
-- KLS-4DP-B2-TempBombSales_Save | 4dp Phase-1 (2026-07-09)
...
--   FactorToBase slot lu.FactorToBase -> lu.ShipQty (entered physical qty, audit). Qty stays lu.BaseShipQty (base
--   physical); Price stays lu.UnitPrice. Audit-only: RecalcQAV values sales as Qty*AvgCost and never reads @INV
--   BillQty/FactorToBase -> zero costing impact (promo/free lines BillQty=0 stay inert). Revenue (@ISALE/@ICREDIT)
--   + @COGS rows keep lu.FactorToBase. No InventoryClear rebuild -> no Option-B/survivor normalization needed.
--   Deploys with the sales group (Sales_Insert + Sales_PartialUpdate).

CREATE OR ALTER PROCEDURE [dbo].[TempBombSales_Save]
    @EmpId INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE
        ...
        @INVAccountId     INT,
        @LockCount        INT,
        @HasInventoryLines BIT = 0,
        @MissingAccounts  NVARCHAR(200);
    ...
    -------------------------------------------------------------------------
    -- 3. Validate base sales posting accounts.
    --    @COGS/@INV are validated later only when changed rows are inventory.
    -------------------------------------------------------------------------
    SET @MissingAccounts = N'';

    IF @ARAccountId IS NULL SET @MissingAccounts += N', @AR';
    IF @FSTPAccountId IS NULL SET @MissingAccounts += N', @FSTP';
    IF @ISALEAccountId IS NULL SET @MissingAccounts += N', @ISALE';
    IF @ICREDITAccountId IS NULL SET @MissingAccounts += N', @ICREDIT';

    IF LEN(@MissingAccounts) > 0
    BEGIN
        SET @MissingAccounts = STUFF(@MissingAccounts, 1, 2, N'');
        RAISERROR('TempBombSales_Save missing required posting account(s): %s', 16, 1, @MissingAccounts);
        RETURN;
    END;

    -------------------------------------------------------------------------
    -- 4. Temp table for Sales header + journal header info.
    -------------------------------------------------------------------------
    ...
    -------------------------------------------------------------------------
    -- 5. Temp table for changed bomb lines.
    ...
    BEGIN TRY
        ...
        ---------------------------------------------------------------------
        -- 8A. Validate inventory posting accounts only when changed rows need them.
        --     Non-inventory bomb saves do not post @COGS/@INV.
        ---------------------------------------------------------------------
        SELECT @HasInventoryLines = CASE
            WHEN EXISTS (SELECT 1 FROM #LineUpdates WHERE ItemType = 'Inventory') THEN 1
            ELSE 0
        END;

        IF @HasInventoryLines = 1
        BEGIN
            SET @MissingAccounts = N'';

            IF @COGSAccountId IS NULL SET @MissingAccounts += N', @COGS';
            IF @INVAccountId IS NULL SET @MissingAccounts += N', @INV';
        ...
        SET
            ...
            -- 2026-07-17 FIX_ICREDIT_AMOUNT_SIGN: Amount = magnitude (ABS), the same value fed
            -- to Fn_CrDeAmount below. OLD: tjd.Amount = lu.ExtTotal
            tjd.Amount       = ABS(lu.ExtTotal),
        ...
        SELECT
            ...
            lu.ExtTotal,
            -- 2026-07-17 FIX_ICREDIT_AMOUNT_SIGN: Amount = magnitude (ABS), the same value fed
            -- to Fn_CrDeAmount below. OLD: lu.ExtTotal,
            ABS(lu.ExtTotal),
            ca.CrDeAmount,
            lu.SalesDetailId,
            lu.FactorToBase
        ...
        UPDATE tjd
        SET
            tjd.Qty          = lu.BaseShipQty,   -- unchanged (base physical)
            tjd.Price        = lu.UnitPrice,     -- unchanged (entered revenue)
            -- 2026-07-07 Effort-B TempBombSales: BillQty base->entered.  OLD: tjd.BillQty      = lu.BaseBillQty,
            tjd.BillQty      = lu.BillQty,
            tjd.ItemId       = lu.NewItemId,
            -- 2026-07-07 Effort-B TempBombSales: FactorToBase slot ratio->EntQty (ShipQty).  OLD: tjd.FactorToBase = lu.FactorToBase
            tjd.FactorToBase = lu.ShipQty
        FROM TransactionJournalDetail tjd
        INNER JOIN #LineUpdates lu
            ON lu.TxId = tjd.TxId
           AND lu.SalesDetailId = tjd.SourceDetailId
        ...
        SELECT
            ...
            lu.PayeeId,
            lu.NewItemId,
            lu.BaseShipQty,          -- Qty: base physical (unchanged)
            lu.UnitPrice,            -- Price: entered revenue (unchanged)
            -- 2026-07-07 Effort-B TempBombSales: BillQty base->entered.  OLD: lu.BaseBillQty,
            lu.BillQty,
            lu.SalesDetailId,
            -- 2026-07-07 Effort-B TempBombSales: FactorToBase slot ratio->EntQty (ShipQty).  OLD: lu.FactorToBase
            lu.ShipQty
        FROM #LineUpdates lu
        WHERE lu.ItemType = 'Inventory'
          AND NOT EXISTS
        ...
        WHERE ISNULL(lu.ItemType, '') <> 'Inventory'
          AND (
                 (@COGSAccountId IS NOT NULL AND tjd.AccountId = @COGSAccountId)
              OR (@INVAccountId IS NOT NULL AND tjd.AccountId = @INVAccountId)
          );
