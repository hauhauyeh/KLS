/*
    Report_SalesSummary
    --------------------
    2026-05-14 - Initial version (new SP, no prior live baseline).

    What it returns
        One row per period over a user-selected date range, with total
        sales for the period and the same period one year prior, plus
        delta amount and delta percent.

    Grain
        @Grain controls the time-bucket:
            day    - one row per calendar day
            week   - one row per Monday-anchored week (ISO week)
            month  - one row per calendar month
            year   - one row per calendar year

    Date bucketed on Sales.SalesDate.

    Sales-role visibility (server-trusted)
        @SalesRepId filters Sales.SalesRepId (transactional credit).
        The C# service overrides @SalesRepId to the caller's EmpId
        when UserContext.IsSalesRole is true, regardless of what the
        request asked for. Mirrors the pattern used by Report_SalesDaily,
        Report_SalesCommission, and the rest of the existing sales-trend
        reports. NULL = no rep filter (admin scope).

    Credit memos
        Sales.SalesTotal is signed (CMs are negative), so SUM(SalesTotal)
        is net. CurrentTx / PriorTx count ALL Sales rows (SO + CM).

    Prior-period pairing (grain-aware)
        day:   PeriodKey = YYYYMMDD encoding of PeriodStart.
               PriorPeriodKey = YYYYMMDD encoding of DATEADD(YEAR,-1,PeriodStart).
               SQL Server clips leap-day Feb-29 to Feb-28 on year-shift,
               so the pairing is well-defined even for leap-edge dates.
        week:  PeriodKey = IsoYear * 100 + IsoWeek (with ISO-year correction
               for late-Dec/early-Jan dates that belong to neighbor ISO year).
               PriorPeriodKey = (IsoYear - 1) * 100 + IsoWeek.
               ISO week numbers repeat year-over-year, so the pairing is
               stable regardless of which Monday the dates land on.
        month: PeriodKey = Year * 100 + Month.
               PriorPeriodKey = (Year - 1) * 100 + Month.
        year:  PeriodKey = Year.
               PriorPeriodKey = Year - 1.

    Empty periods
        Periods calendar CTE drives the row template. Periods with no
        sales in the current window still produce a row with
        CurrentTotal = 0 (zero-fill). DeltaPercent is NULL when
        PriorTotal = 0 so the frontend can render as 'n/a'.

    Sort
        PeriodStart ASC (chronological).

    Defaults (when caller passes NULL date range)
        day   -> last 30 days
        week  -> last 13 weeks
...
BEGIN
    ...
    IF @Grain NOT IN ('day', 'week', 'month', 'year')
        THROW 50020, 'Invalid @Grain. Use ''day'', ''week'', ''month'', or ''year''.', 1;
