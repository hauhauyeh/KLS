...
BEGIN
    ...
    (
        SELECT AccountId
        FROM Account a
        INNER JOIN AccountCategory c ON a.AccountCategoryId = c.AccountCategoryId
        WHERE c.ClassCode = 'I'
    ...
    (
        SELECT
            ...
            t.SourceDocNumber,
            CASE
                WHEN td.AccountId = @COGSAccountId THEN @ISALEAccountId
                ELSE td.AccountId
            END AS AccountId,
            SUM(CASE WHEN td.AccountId != @COGSAccountId THEN td.Amount ELSE 0 END) AS SalesTotal,
        ...
        INNER JOIN IncomeAccounts ia ON ia.AccountId = td.AccountId
    ...
    (
        ...
        INNER JOIN CustomerPaymentDetail pd ON pd.CustomerPaymentId = c.CustomerPaymentId
    ...
    SELECT
        ...
        s.SalesRepId,
        f.SalesTotal,
        CAST(0 AS DECIMAL(18,4)) AS SalesPercent,
        f.Cost,
        CAST(0 AS DECIMAL(18,4)) AS MarginPercent,
        f.AccountId,
        c.AccountName
    INTO #DailySales
    FROM FinalData f
    INNER JOIN Sales s ON s.SalesNumber = f.SourceDocNumber
    INNER JOIN Payee p ON p.PayeeId = s.ShipId
    INNER JOIN Account c ON c.AccountId = f.AccountId
    ...
    SET
        SalesPercent = CASE
            WHEN @TotalSales != 0 THEN SalesTotal / @TotalSales
            ELSE 0
        END,
        MarginPercent = CASE
            WHEN SalesTotal != 0 THEN (SalesTotal - Cost) / SalesTotal
            ELSE 0
        END;
