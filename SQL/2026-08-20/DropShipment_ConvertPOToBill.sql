...
CREATE OR ALTER PROCEDURE [dbo].[DropShipment_ConvertPOToBill] -- EXEC dbo.DropShipment_ConvertPOToBill @PurchaseId=0,@EmpId=0,@VendorDocNumber='VDOC',@ContainerNumber='CONT'
    @PurchaseId INT,
    @EmpId INT,
...
BEGIN
    ...
    -- Slice 4: convert is allowed after customer receipt when the DS PO has factory progress.
    -- Stage 2 = partially shipped, stage 3 = shipped. Stage 1 has no factory-progress signal.
    IF @PurchaseStageId NOT IN (2, 3)
    BEGIN
