...
BEGIN
    ...
    SET StageId =
        CASE
            ...
            -- Drop-ship pre-bill PO stage is owned by DropShipment_SyncSalesTransitFromPO below.
            -- Keep the current DS stage during the shared normal-PO stage update so the normal
            -- quantity/receipt rules do not compete with the DS factory-progress rule.
            WHEN ISNULL(p.IsDropShip, 0) = 1
              AND p.StageId IN (1, 2, 3, 4, 5)
            THEN p.StageId

            -- Replaced DS special case:
            -- WHEN ISNULL(p.IsDropShip, 0) = 1 AND any item ShipQty > 0 THEN 2
            -- WHEN ISNULL(p.IsDropShip, 0) = 1 THEN 1
