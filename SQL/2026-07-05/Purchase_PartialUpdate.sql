-- =============================================================================
-- Purchase_PartialUpdate -- deploy (CREATE OR ALTER). Working file per SP workflow.
-- 2026-07-05: Phase B -- thread rational-unit conversion into the base-qty producer.
--   Base qty derives from the line's ItemUnitId -> ItemUnit (single source of truth) via
--   dbo.Fn_QtyToBase(qty, MultipleToBase, FactorToBase), retiring the FactorToBase snapshot read.
--   Loop producer: scalar SUBQUERY form (NULL on unresolved ItemUnitId -> base qty NULL for account
--   lines; do NOT use 'SELECT @x=col FROM ItemUnit WHERE...' which keeps the previous row on no-match).
--   Whole-row staleness is impossible: @TempTable.AutoId is IDENTITY(1,1), single bulk insert, never
--   deleted -> contiguous 1..N, so the row-fetch always matches exactly one row.
--   Post-loop bulk INNER JOIN refresh makes ALL rows (incl. unchanged survivors) ItemUnit-consistent
--   (parity with Sales_PartialUpdate; no-op today, forward-correct for combine-up).
--   QTY ONLY: @FactorToBase snapshot kept for the @INV price / journal FactorToBase (price deferred).
-- Baseline: KLS/SQL/2026-07-05/Purchase_PartialUpdate_live_baseline.sql
-- =============================================================================
SET ANSI_NULLS ON
GO
