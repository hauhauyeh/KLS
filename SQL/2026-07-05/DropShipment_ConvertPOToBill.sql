-- =============================================================================
-- DropShipment_ConvertPOToBill -- deploy (CREATE OR ALTER). Working file per SP workflow.
-- 2026-07-05: Phase B -- thread rational-unit conversion into the PURCHASE-side base-qty producer.
--   Sales side (SalesDetail) already threaded 2026-07-04 (commit 2b2a6c2) -- untouched here.
--   STEP 1's two branch UPDATEs (Order stage / Transit stage) kept their entered-qty fills; the
--   base-qty lines moved to ONE shared producer after the IF/ELSE that derives BaseReceiveQty/
--   BaseFinalQty from ItemUnitId -> ItemUnit via dbo.Fn_QtyToBase + INNER JOIN ItemUnit.
--   INNER JOIN safe: account lines (no ItemUnitId) don't consume base qty on the purchase side
--   (@APDS is LineType<>'A'; account rows use FinalQty*FinalPrice). Qty-only: drop-ship posts
--   @APDS/@AP (no @INV), @APDS price has no factor conversion; FactorToBase snapshot stays.
--   Identity today (all Mult=1). Runs before STEP 2 reads pd.BaseFinalQty.
-- Baseline: KLS/SQL/2026-07-05/DropShipment_ConvertPOToBill_live_baseline.sql
-- =============================================================================
SET ANSI_NULLS ON
GO
...
-- DropShipment_ConvertPOToBill
-- Converts a drop-ship purchase from PO (StageId=1) to Bill (StageId=6)
-- and posts both purchase-side and sales-side journal entries in one transaction.
--
-- Purchase-side: DR @APDS, CR @AP (no inventory)
-- Sales-side: DR @AR, CR Revenue, DR @COGS, CR @APDS (no inventory, no FIFO)
--
-- No RecalcQAV, no FIFO allocation, no shipment allocation.
...
BEGIN
    ...
    INSERT INTO @AcctTable(AccountCode) VALUES('@APDS');
    ...
    BEGIN TRY
        ...
        -- not consumed on the purchase side (@APDS is LineType<>'A'; account rows use FinalQty*FinalPrice).
        UPDATE pd
        ...
        -- ============================================================
        -- STEP 2: Purchase-side posting (DR @APDS, CR @AP)
        -- ============================================================
        ...
        -- @APDS per-line rows (debit side) �?" one row per PurchaseDetail line
        INSERT INTO #PurTxDetail
        ...
        SELECT
            ...
            pd.FactorToBase
        FROM PurchaseDetail pd
        CROSS APPLY (SELECT AccountId FROM @AcctTable WHERE AccountCode = '@APDS') apds
        CROSS APPLY dbo.Fn_CrDeAmount
		(
			CONVERT(NVARCHAR(20), apds.AccountId),
        ...
        -- ============================================================
        -- STEP 3: Sales-side posting (DR @AR, CR Revenue, DR @COGS, CR @APDS)
        -- ============================================================
        ...
        -- CR @APDS for COGS (clearing account nets to zero)
        SELECT @AccountId = AccountId FROM @AcctTable WHERE AccountCode = '@APDS';
        EXEC Fn_Adjust_CrDeAmount @AccountId, @COGSTotal, @CrDeAmount OUTPUT;
