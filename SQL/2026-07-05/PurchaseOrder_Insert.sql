-- =============================================================================
-- PurchaseOrder_Insert -- deploy (CREATE OR ALTER). Working file per SP workflow.
-- 2026-07-05: Phase B -- thread rational-unit conversion into the base-qty producer.
--   Section-6 bulk UPDATE now derives BaseReceiveQty/BaseFinalQty from the line's
--   ItemUnitId -> ItemUnit via dbo.Fn_QtyToBase, retiring the FactorToBase snapshot read.
--   Set-based INNER JOIN = item lines only; account lines (no ItemUnitId) are not updated
--   (keep existing base qty). Shared after both branches (IF @PurchaseId>0 edit / ELSE new,
--   @PurchaseId = SCOPE_IDENTITY()), keyed by @PurchaseId -- correct for both flows.
--   Qty-only: no @INV journal on a PO (placeholder @AP row only), so no price line to thread;
--   FactorToBase snapshot column stays. PO base qty IS consumed downstream (PO->Bill, history,
--   incoming-stock), so making it ItemUnit-derived is the point. Identity today (all Mult=1).
-- Baseline: KLS/SQL/2026-07-05/PurchaseOrder_Insert_live_baseline.sql
-- =============================================================================
SET ANSI_NULLS ON
GO
