-- =============================================================================
-- VendorPayment_InsertPayNow -- deploy (CREATE OR ALTER). Working file per SP workflow.
-- 2026-07-05: Phase B -- thread rational-unit conversion into the base-qty producer.
--   The producer CTE UPDATE did LineId renumber + base qty together. Split: CTE keeps LineId;
--   base qty moves to a dedicated UPDATE (before @MyTable is loaded).
--   ITEM lines derive base qty from ItemUnitId -> ItemUnit via dbo.Fn_QtyToBase. ACCOUNT lines
--   keep the snapshot formula via LEFT JOIN + CASE fallback -- they CONSUME @BaseFinalQty (the
--   account journal row's Qty, loop ~line 504), so INNER JOIN would NULL it. Account FactorToBase=1
--   => fallback = FinalQty, byte-identical. The ELSE branch is the OLD formula verbatim.
--   Qty-only: @INV price (@MyFinalPrice * @FactorToBase) + journal FactorToBase keep the snapshot
--   factor; price threading deferred to combine-up. Loop (consumer) untouched. Identity today (Mult=1).
-- Baseline: KLS/SQL/2026-07-05/VendorPayment_InsertPayNow_live_baseline.sql
-- =============================================================================
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	-- branch is the OLD formula verbatim. @FactorToBase snapshot stays for the @INV price. Prior (in CTE):
