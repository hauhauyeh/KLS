-- =============================================================================
-- Purchase_Insert -- deploy (CREATE OR ALTER). Working file per SP workflow.
-- 2026-07-05: Phase B -- thread rational-unit conversion in the Section-8 base-qty producer.
--   Base qty now derives ONLY from the line's ItemUnitId -> ItemUnit (single source of
--   truth): BaseQty = qty * MultipleToBase / FactorToBase via dbo.Fn_QtyToBase (inlined).
--   ItemUnit ratios are immutable, so the join is authoritative and the pd.FactorToBase
--   snapshot read is retired here. INNER JOIN = item lines only; account lines (LineType='A',
--   no ItemUnitId) are dropped from the base-qty UPDATE (they feed no @INV row).
--   QTY ONLY (mirrors the Sales_Insert pilot): the Section-11 @INV Price still reads
--   pd.FactorToBase; threading price via ItemUnit is deferred to the combine-up (xN) work.
--   Identity today (all MultipleToBase=1 -> qty/factor). Insert-only SP (@IsEdit hardcoded 0);
--   fresh TempPurchase rows, verified 0 orphan ItemUnitId so the INNER JOIN drops nothing.
-- Baseline: KLS/SQL/2026-07-05/Purchase_Insert_live_baseline.sql
-- =============================================================================
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN TRY
     ...
     SELECT
		...
		-- => qty/factor). QTY ONLY: the Section-11 @INV Price still reads pd.FactorToBase; when
		-- combine-up (xN) lands it must become FinalPrice * factor / multiple. Prior block:
	...
	(
		...
		SELECT
			...
			pd.ItemId,
			pd.BaseReceiveQty,
			ROUND(ROUND(pd.FinalPrice,2) * pd.FactorToBase,2),
			pd.BaseFinalQty,
			pd.PurchaseDetailId,
			pd.FactorToBase
		FROM PurchaseDetail pd
