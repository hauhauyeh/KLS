...
--
-- Results are materialized into #flagged so BOTH the summary and the detail SELECT can
-- read them (a CTE is only visible to the single statement that immediately follows it).
-- =============================================================================
...
BEGIN
    ...
    -- Build the per-line three-truth comparison, with break flags, into #flagged.
    ;WITH book AS (
        -- Book Truth: net posted @INV base qty per line (SUM handles multi-row / adjustments).
        SELECT tjd.SourceDetailId AS SalesDetailId, book_qty = SUM(ISNULL(tjd.Qty, 0))
    ...
    SELECT
        SalesDetailId, SalesId, ItemCode, ItemType, ShipDate,
