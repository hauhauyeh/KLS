...
CREATE OR ALTER PROCEDURE [dbo].[VendorPayment_Import] --[PayNow_Import] 'C:\Users\Admin\Downloads\CC (1).xlsx'
...
BEGIN
    ...
    ''Excel 12.0; HDR=yes; Database='+CONVERT(NVARCHAR(255),@FilePath)+''', [Sheet1$]);'
		...
		-- Read the workbook BEFORE opening the transaction. An ad-hoc
		-- OPENROWSET inside an explicit transaction risks MSDTC promotion.
	...
	BEGIN CATCH
		...
		-- Re-raise the original error (number, message, line) so the
		-- caller sees exactly what it saw before this change.
		THROW;
