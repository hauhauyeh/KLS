...
-- =====================================================================
CREATE   PROCEDURE [dbo].[Shipment_BillBasisUsability]
    @ShipmentId INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    ;WITH L AS (
        ...
        JOIN   dbo.Purchase pr ON pr.PurchaseId = sp.PurchaseId   -- 2026-07-13 DROPSHIP-EXCLUDE (Slice B)
        WHERE  sp.ShipmentId = @ShipmentId
          AND  ISNULL(pr.IsDropShip, 0) = 0
    ),
    E AS (
    ...
    SELECT
        ...
        IsDropShip            = pr2.IsDropShip   -- 2026-07-13 DROPSHIP-EXCLUDE (Slice B)
    FROM   dbo.ShipmentPurchase sp
