...
BEGIN
    ...
    DECLARE @IsDropShipExisting BIT = 0;
    ...
    BEGIN
        ...
        SELECT @IsDropShipExisting = ISNULL(IsDropShip, 0)
        FROM Purchase
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            -- Drop-ship PO item rows are mapped back to SalesDetail by LineId + ItemId.
            -- Factory-side PO edit may change quantities/prices, but must not change item structure.
            IF @IsDropShipExisting = 1
               AND EXISTS
               (
                   SELECT 1
                   FROM TempPurchase t
                   WHERE t.PayeeId = @PayeeId
                     ...
                     AND t.LineType = 'I'
                     AND t.ChangeStatus IN ('I', 'D')
               )
            BEGIN
                RAISERROR('Drop-ship PO edit cannot add or delete item lines.', 16, 1);
                RETURN;
            END
            ...
            BEGIN
                RAISERROR('Drop-ship PO edit cannot reorder item lines.', 16, 1);
                RETURN;
            END

            DELETE pd
            FROM PurchaseDetail as pd inner join TempPurchase as t ON t.PurchaseDetailId=pd.PurchaseDetailId 
            WHERE t.PayeeId = @PayeeId AND t.EmpId = @EmpId AND t.PurchaseId = @PurchaseId AND t.ChangeStatus = 'D';
            ...
            SET  LineId            = t.LineId
                ...
                ,BillQty           = t.BillQty
                ,OrdQty1           = t.OrdQty1
                -- Drop-ship PO edit owns factory qty only; customer receipt stays with Order Manager.
                ,ReceiveQty        = CASE WHEN @IsDropShipExisting = 1 THEN pd.ReceiveQty ELSE t.ReceiveQty END
                ,FinalQty          = CASE WHEN @IsDropShipExisting = 1 THEN pd.FinalQty ELSE t.FinalQty END
            ...
            FROM PurchaseDetail as pd inner join TempPurchase as t ON t.PurchaseDetailId=pd.PurchaseDetailId 
            WHERE t.PayeeId = @PayeeId AND t.EmpId = @EmpId AND t.PurchaseId = @PurchaseId AND t.ChangeStatus = 'U';
            ...
            SELECT 
                ...
                [BillQty],
                [OrdQty1],
                CASE WHEN @IsDropShipExisting = 1 THEN NULL ELSE [ReceiveQty] END,
                CASE WHEN @IsDropShipExisting = 1 THEN NULL ELSE [FinalQty] END,
                [BillPrice],
                [BillExtTotal],
                [FinalPrice],
                [ImportCommission],
                ...
                [ItemVolume]
            FROM TempPurchase 
            WHERE PayeeId = @PayeeId AND EmpId = @EmpId AND PurchaseId = @PurchaseId AND ChangeStatus = 'I';
