...
-- 2026-07-14 DSCC-AMT-SIGN (marker DSCC_AMT_SIGN_20260714): sales-side CR @DSCC clearing row now stores the
--   SIGNED source value (Amount = -COGSTotal) instead of the positive magnitude, so Amount and CrDeAmount come
--   from the same value (posting rule: Amount is source of truth, CrDeAmount derived from it; RecalcQAV pattern).
--   CrDeAmount is unchanged. No data repair script is kept in this folder.
-- 2026-07-14 PUR-AMT-SIGN (marker PUR_AMT_SIGN_20260714): purchase-side line rows now keep the SIGNED Amount
--   (drop forced ABS); Amount = signed growth of the row's account in its normal-balance direction, and
--   CrDeAmount derives from that SAME value: (1) @DSCC per-line rows - Amount and the Fn_CrDeAmount input use
--   the same signed ROUND(FinalQty*FinalPrice,2); (2) account-type 'A' lines - restore the original pre-ABS
--   CASE (debit-nature +value / credit-nature -value) and derive CrDe via Fn_CrDeAmount CROSS APPLY (the old
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            ...
            pd.BaseFinalQty,
            -- was (2026-07-13 DSCC-FIX, sign-blind): ABS(ROUND(pd.FinalQty * pd.FinalPrice, 2))
            --   (that fix corrected an older bug that stored crde.CrDeAmount in Amount, but forced ABS)
            -- 2026-07-14 PUR-AMT-SIGN: Amount = SIGNED line value (source of truth); the Fn_CrDeAmount input
            --   below uses the SAME signed value (debit-nature @DSCC: positive -> debit, negative -> credit),
            --   so a negative line stays balanced against @AP, whose total already includes it signed.
            ROUND(pd.FinalQty * pd.FinalPrice, 2),  -- PUR_AMT_SIGN_20260714
            crde.CrDeAmount,
            pd.PurchaseDetailId,
            pd.FactorToBase
        FROM PurchaseDetail pd
		...
		(
			...
			-- was (2026-07-13 DSCC-FIX): ABS(ROUND(pd.FinalQty * pd.FinalPrice,2))  -- +input -> debit; before that -ABS for credit-nature @APDS
			ROUND(pd.FinalQty * pd.FinalPrice,2)  -- 2026-07-14 PUR-AMT-SIGN: signed input, same value as Amount
		) crde
        WHERE pd.PurchaseId = @PurchaseId
          AND pd.LineType <> 'A';
        ...
        -- EXEC args cannot be expressions, so negate via a variable.
        -- was (2026-07-13, WRONG): "The stored Amount below stays +@COGSTotal."
        -- 2026-07-14 DSCC-AMT-SIGN: Amount is the posting source of truth and CrDeAmount must be derived from that
        --   SAME value (RecalcQAV pattern: set signed Amount first, derive CrDeAmount from it, store both together).
        --   This row REDUCES the @DSCC clearing asset, so its source value is negative: store @NegCOGSTotal in
        --   Amount. @DSCC rows now net to zero in Amount as well as CrDeAmount. CrDeAmount itself is unchanged.
        ...
        INSERT INTO #SalTxDetail
            ([SortGroup], [LineSort], [RowSort], [TxId], [AccountId], [PayeeId],
             [Amount], [CrDeAmount])
        VALUES
            -- (2, 0, 2, @SalTxId, @AccountId, @PayeeId, @COGSTotal, @CrDeAmount);  -- 2026-07-14 DSCC-AMT-SIGN: was +@COGSTotal while CrDe came from @NegCOGSTotal (wrong Amount sign)
            (2, 0, 2, @SalTxId, @AccountId, @PayeeId, @NegCOGSTotal, @CrDeAmount);  -- DSCC_AMT_SIGN_20260714
