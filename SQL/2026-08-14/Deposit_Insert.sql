...
GO
CREATE OR ALTER PROCEDURE [dbo].[Deposit_Insert] -- DECLARE @NewTFId INT; EXEC dbo.Deposit_Insert @TFId=0, @TFDate='2026-08-14', @ToAccountId=1, @TransferAmount=100, @CashbackAccountId=NULL, @CashbackAmount=0, @CCFeeAmount=0, @RoundingOff=0, @EmpId=100001, @NewTFId=@NewTFId OUTPUT
...
BEGIN
	...
	-- 2026-08-14: check only the staging set this call consumes (same TFId predicate as the
	-- TransferFundDetail INSERT below). Residue rows from a merely-viewed deposit (TFId>0,
	-- seeded by Deposit_Inject) reference payments that are IsLocked=1 BECAUSE they are
	-- deposited - counting them here false-positived every new deposit for that employee.
	...
	SELECT @custpmtcount=COUNT(CustomerPaymentId) FROM CustomerPayment WHERE CustomerPaymentId IN (SELECT CustomerPaymentId FROM TempTransferFund WHERE EmpId=@EmpId AND TFId=(CASE WHEN @IsEdit=0 THEN 0 ELSE @OldTFId END) AND IsApplied=1) AND IsLocked=1
	...
	-- 2026-08-14: delete only the staging set this call consumed (same predicate the
	-- TransferFundDetail INSERT above reads, minus IsApplied so unselected candidates go
	-- too). The old emp-wide delete also destroyed TempTransferFund rows belonging to a
	-- DIFFERENT TFId - e.g. a manual edit session's staging - whenever Bank Feed created
	-- a deposit (TFId=0) for the same employee.
	--DELETE TempTransferFund where EmpId=@EmpId
	DELETE TempTransferFund
	WHERE EmpId=@EmpId
	  AND TFId=(CASE WHEN @IsEdit=0 THEN 0 ELSE @OldTFId END)
