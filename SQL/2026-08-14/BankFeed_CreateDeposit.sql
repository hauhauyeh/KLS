...
CREATE OR ALTER PROCEDURE [dbo].[BankFeed_CreateDeposit]   -- EXEC dbo.BankFeed_CreateDeposit @BankFeedTransactionId=1, @PaymentIdsJson='[1,2]', @DifferenceKind='None', @DifferenceAccountId=NULL, @DifferenceMemo=NULL, @EmpId=1, @NewTFId=NULL
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 2026-08-14: narrowed to TFId=0. Deposit_Inject seeds IsApplied=1 rows stamped with
        -- the deposit's TFId whenever an EXISTING deposit is merely opened on the manual
        -- screen, and nothing clears them on close - so the old any-TFId guard blocked the
        -- next Bank Feed deposit after simply viewing one (false positive). Only TFId=0
        -- rows are a new-deposit draft that Deposit_Insert (@TFId=0) would absorb; TFId>0
        -- rows are edit/view staging this path never reads, and (paired change, same day)
        -- Deposit_Insert now deletes only the TFId set it consumed, so they survive intact.
        --IF EXISTS (SELECT 1 FROM dbo.TempTransferFund
        --           WHERE EmpId = @EmpId AND IsApplied = 1)
        IF EXISTS (SELECT 1 FROM dbo.TempTransferFund
                   WHERE EmpId = @EmpId AND TFId = 0 AND IsApplied = 1)
        ...
        --    (EmpId + TFId=0 + IsApplied=1). Deposit_Insert deletes the TFId=0 set for
        --    @EmpId on success (2026-08-14, was all rows) - do not delete again after E.
        --    2026-08-14: pre-clean narrowed to TFId=0 to match - TFId>0 rows are manual
        --    edit/view staging and must survive this path.
        -----------------------------------------------------------------------------------
        --DELETE FROM dbo.TempTransferFund WHERE EmpId = @EmpId;
        DELETE FROM dbo.TempTransferFund WHERE EmpId = @EmpId AND TFId = 0;
