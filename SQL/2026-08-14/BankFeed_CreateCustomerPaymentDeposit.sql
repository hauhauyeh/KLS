...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 2026-08-14: narrowed to TFId=0, same fix as BankFeed_CreateDeposit's 50413 guard.
        -- Merely opening an existing deposit on the manual screen seeds IsApplied=1 rows
        -- stamped with that TFId (Deposit_Inject) and nothing clears them on close, so the
        -- any-TFId guard false-positived after "create deposit -> view it -> create next".
        -- Only TFId=0 rows are a draft Deposit_Insert (@TFId=0) would absorb; TFId>0 rows
        -- survive this path now that Deposit_Insert (paired change) deletes only its own set.
        ...
        -- 2026-08-14: pre-clean narrowed to TFId=0 to match the guard - TFId>0 rows are
        -- manual edit/view staging and must survive this path.
