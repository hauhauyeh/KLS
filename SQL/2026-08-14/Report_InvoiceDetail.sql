...
GO
-- 2026-08-07 DropShip invoice Slice 5: disable packed retail grouping and hide zero-ship drop-ship item lines.
...
BEGIN
    ...
    DECLARE @PackedItemGroupEnabled BIT = 0;

    SELECT @ITEM_DEFAULT_SORTORDER=SettingValue FROM SystemSetting WHERE SettingKey = 'ITEM_DEFAULT_SORTORDER'

    SELECT @IsDropShip = ISNULL(s.IsDropShip, 0)
    FROM dbo.Sales AS s
    WHERE s.SalesId = @SalesId;

    SELECT @PackedItemGroupEnabled =
        CASE
            WHEN LOWER(LTRIM(RTRIM(SettingValue))) IN ('true', '1', 'yes', 'y') THEN 1
            ELSE 0
        END
    ...
    -- 2026-08-14: Restore old PACKED ITEM grouping only when the KLS setting is enabled.
    IF @PackedItemGroupEnabled = 1
    BEGIN
