...
GO
-- =============================================================================================
-- Tariff Phase 2 - ItemTariff_GetAllList
-- Date : 2026-07-23   Plan: plan/plan-tariff-phase-2-admin-module.md
-- Change from baseline: add ItemCode, HSNCode, Notes, UpdatedAt to the result set;
--   extend search to also match HSNCode and CountryName. "Updated" is
--   ISNULL(UpdatedAt, CreatedAt) so the column always shows a last-changed date.
-- NOTE: result columns MUST stay in lockstep with KLS.Models.ItemTariffList.
-- =============================================================================================
	...
	ELSE
		SET @Qry = '
			SELECT
				...
				,it.TariffRate
				,it.Notes
				,UpdatedAt = ISNULL(it.UpdatedAt, it.CreatedAt)';
	...
	BEGIN
		...
		SET @Qry += '
			AND (
				...
				i.ItemName LIKE ''%' + CONVERT(NVARCHAR(100), @Search) + '%'' OR
				it.HSNCode LIKE ''%' + CONVERT(NVARCHAR(100), @Search) + '%'' OR
				c.CountryName LIKE ''%' + CONVERT(NVARCHAR(100), @Search) + '%''
			) ';
	END
