...
CREATE OR ALTER PROCEDURE [dbo].[EmailLog_GetAllList]
	@Pageno int,
	@Pagesize int,
	@Search nvarchar(100),
	@StartDate date,
...
BEGIN
	...
	IF @EmailType IS NOT NULL
		-- 2026-07-23: include legacy rows where EmailType was not backfilled and EventType carries the type.
		-- SET @Where += N' AND E.EmailType = @EmailType ';
		-- 2026-07-23: include document-type matches too; web-order invoice emails use WebOrder* as EmailType and Invoice as DocumentType.
		-- SET @Where += N' AND (E.EmailType = @EmailType OR (E.EmailType IS NULL AND E.EventType = @EmailType)) ';
		SET @Where += N' AND (E.EmailType = @EmailType OR E.DocumentType = @EmailType OR (E.EmailType IS NULL AND E.EventType = @EmailType)) ';
