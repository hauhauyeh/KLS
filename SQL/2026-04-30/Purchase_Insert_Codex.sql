...
BEGIN
	...
	(
		...
		SELECT
			...
			ABS(CASE WHEN a.IsAccountDebit = 1 THEN ROUND(pd.FinalQty * pd.FinalPrice,2) ELSE -ROUND(pd.FinalQty * pd.FinalPrice,2) END),
			CASE WHEN a.IsAccountDebit = 1 THEN ROUND(pd.FinalQty * pd.FinalPrice,2) ELSE -ROUND(pd.FinalQty * pd.FinalPrice,2) END,
			CASE WHEN a.IsAccountDebit = 1 THEN -ABS(ROUND(pd.FinalQty * pd.FinalPrice,2)) ELSE -ABS(ROUND(pd.FinalQty * pd.FinalPrice,2)) END,
			pd.PurchaseDetailId
		...
		SELECT
			...
			CASE WHEN cogsAcct.IsAccountDebit = 1 THEN -ROUND(pd.FinalQty * pd.FinalPrice,2) ELSE ROUND(pd.FinalQty * pd.FinalPrice,2) END,
			pd.PurchaseDetailId,
			pd.BaseReceiveQty
		FROM PurchaseDetail pd
		INNER JOIN Item i ON i.ItemId = pd.ItemId
		CROSS JOIN (SELECT AccountId FROM @AcctTable WHERE AccountCode='@COGS') cogs
		INNER JOIN Account cogsAcct ON cogsAcct.AccountId = cogs.AccountId
		WHERE pd.PurchaseId = @PurchaseId
		  AND pd.LineType <> 'A'
