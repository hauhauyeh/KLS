...
-- 3. Candidate supporting index is included in the same draft for review; deploy only after validating wider write impact.
...
BEGIN
    CREATE INDEX IX_TransactionJournalDetail_AccountId_ItemId_TxId
    ON dbo.TransactionJournalDetail (AccountId, ItemId, TxId)
    INCLUDE (TxDetailId, Qty, Price, BillQty);
END
GO
