...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            IF @SalesId IS NULL
            BEGIN
                RAISERROR('Sales header insert failed.', 16, 1);
                RETURN;
            END

            -- Insert SalesDetail from TempSales
            ...
            SELECT
                ...
                [ItemUnitId], [Unit], [OrdQty], [ShipQty], [BillQty],
                [UnitPrice],
                ROUND(ISNULL(BillQty, 0) * ISNULL(UnitPrice, 0), 2),
                [Notes],
                CASE
            ...
            IF @@ROWCOUNT = 0
            BEGIN
                RAISERROR('No detail rows inserted from TempSales.', 16, 1);
                RETURN;
            END
        END
        ELSE
        BEGIN
            ...
            IF @@ROWCOUNT = 0
            BEGIN
                RAISERROR('No detail rows inserted from TempSales (edit).', 16, 1);
                RETURN;
            END
        ...
        IF @@ROWCOUNT = 0
        BEGIN
            RAISERROR('No purchase detail rows inserted from TempPurchase.', 16, 1);
            RETURN;
        END

        -- Calculate purchase totals from PurchaseDetail
        SELECT @BillTotal = ISNULL(SUM(ROUND(ISNULL(OrdQty0, 0) * ISNULL(BillPrice, 0), 2)), 0)
        FROM PurchaseDetail
        WHERE PurchaseId = @PurchaseId;

        UPDATE Purchase SET
            VendorTotal = @BillTotal,
            PurchaseTotal = @BillTotal,
            AmountDue = @BillTotal
        WHERE PurchaseId = @PurchaseId;
