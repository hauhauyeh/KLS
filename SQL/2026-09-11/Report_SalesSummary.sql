...
CREATE OR ALTER PROCEDURE [dbo].[Report_SalesSummary] -- EXEC dbo.Report_SalesSummary @Grain='year', @StartDate='2026-01-01', @EndDate='2026-12-31', @SalesRepId=NULL
...
BEGIN
    ...
    -- Sales Summary is now ship-date based. CurrentTotal/PriorTotal
    -- mean completed sales only (Received + Success); open Order and
    -- Transit amounts are returned separately for current-period review.
    -- Previous behavior used SalesDate and all stages in CurrentTotal.
    ;WITH Cur AS
    (
        SELECT
            x.PeriodKey,
            TotalAmount  = SUM(CASE WHEN s.StageId IN (3, 4) THEN s.SalesTotal ELSE 0 END),
            TxCount      = COUNT(CASE WHEN s.StageId IN (3, 4) THEN s.SalesId END),
            OrderTotal   = SUM(CASE WHEN s.StageId = 0 THEN s.SalesTotal ELSE 0 END),
            OrderTx      = COUNT(CASE WHEN s.StageId = 0 THEN s.SalesId END),
            TransitTotal = SUM(CASE WHEN s.StageId = 2 THEN s.SalesTotal ELSE 0 END),
            TransitTx    = COUNT(CASE WHEN s.StageId = 2 THEN s.SalesId END)
        FROM dbo.Sales s
        CROSS APPLY (
            SELECT PeriodKey =
                CASE @Grain
                    WHEN 'day'   THEN CAST(YEAR(s.ShipDate) AS BIGINT) * 10000 + MONTH(s.ShipDate) * 100 + DAY(s.ShipDate)
                    WHEN 'week'  THEN
                        (CAST(
                            CASE
                                WHEN DATEPART(ISO_WEEK, s.ShipDate) >= 52 AND MONTH(s.ShipDate) = 1  THEN YEAR(s.ShipDate) - 1
                                WHEN DATEPART(ISO_WEEK, s.ShipDate) = 1   AND MONTH(s.ShipDate) = 12 THEN YEAR(s.ShipDate) + 1
                                ELSE YEAR(s.ShipDate)
                            END AS BIGINT) * 100) + DATEPART(ISO_WEEK, s.ShipDate)
                    WHEN 'month'   THEN CAST(YEAR(s.ShipDate) AS BIGINT) * 100 + MONTH(s.ShipDate)
                    WHEN 'quarter' THEN CAST(YEAR(s.ShipDate) AS BIGINT) * 10  + DATEPART(QUARTER, s.ShipDate)
                    WHEN 'year'    THEN CAST(YEAR(s.ShipDate) AS BIGINT)
                END
        ) x
        WHERE s.ShipDate >= @StartDate
          AND s.ShipDate <  DATEADD(DAY, 1, @EndDate)
          AND s.ShipDate >= @DataFloor
          AND s.StageId IN (0, 2, 3, 4)
          AND (s.DocType IS NULL OR s.DocType <> 'OB')
    ...
    (
        ...
        WHERE s.ShipDate >= DATEADD(YEAR, -1, @StartDate)
          AND s.ShipDate <  DATEADD(YEAR, -1, DATEADD(DAY, 1, @EndDate))
          AND s.ShipDate >= @DataFloor
    ...
    SELECT
        ...
        ISNULL(c.TransitTotal, 0)                           AS TransitTotal,
        ISNULL(c.OrderTx, 0)                                AS OrderTx,
        ISNULL(c.TransitTx, 0)                              AS TransitTx
    FROM #Periods pr
