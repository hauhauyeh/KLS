...
CREATE OR ALTER PROCEDURE dbo.Purchase_DropShipPORestrictedUpdate
    @PurchaseId INT,
    @EmpId INT,
    ...
    BEGIN TRY
        ...
        SELECT
            ...
            HasProtectedChange = CONVERT(BIT, CASE WHEN
                ...
                OR (
                    ISNULL(c.IsOut,0) <> ISNULL(s.IsOut,0)
                    AND NOT (
                        ISNULL(s.IsOut,0) = 1
                        AND ISNULL(c.IsOut,0) = 0
                        AND ISNULL(c.ShipQty,0) >= 0
                        AND ISNULL(c.BillQty,0) = ISNULL(c.ShipQty,0)
                    )
                )
        ...
        IF EXISTS (SELECT 1 FROM #ChangedRows WHERE HasInvalidShipQty = 1)
            RAISERROR('ShipQty cannot be negative.', 16, 1);
        ...
        -- Restricted drop-ship vendor price uses one editable value carried in BillPrice and FinalPrice.
