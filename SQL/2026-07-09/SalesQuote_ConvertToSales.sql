...
--   transaction + applock, THROWs on failure). No extra outer
...
--   created; the only residue is the SalesId=0 pre-fill cart,
...
BEGIN
    ...
    SELECT @PayeeId = PayeeId, @QuoteNumber = QuoteNumber
    FROM SalesQuote
    ...
    EXEC [Sales_Insert]
        ...
        @StageId        = 0,              -- standard new-order draft stage (matches Sales/Checkout)
        @EmpId          = @EmpId,
    ...
    IF @NewSalesId > 0
    BEGIN
        SELECT @NewSalesNumber = SalesNumber FROM Sales WHERE SalesId = @NewSalesId;
