...
GO
-- KLS-4DP-B5-TempBombSales_Inject: 4-decimal pricing Section B Phase-1 (storage/type widen, inert).
--   Widen @Price 2dp -> 4dp. @Price is a filter predicate (sd.UnitPrice = @Price) against the Section A-widened
--   SalesDetail.UnitPrice; widening lets a future 4dp price match instead of truncating to 2dp. Inert today:
--   values are <=2dp and CONVERT(VARCHAR,@Price) still parses as the same numeric literal.
CREATE OR ALTER PROCEDURE [dbo].[TempBombSales_Inject]
