...
--   Price       = ROUND(pd.FinalPrice,2)  -- drop the old (× pd.FactorToBase); goods price is entered.
...
BEGIN
	...
	BEGIN TRY
     ...
     SELECT
		...
		-- The Section-11 @INV Price now DROPS × pd.FactorToBase (entered FinalPrice); BillQty -> FinalQty;
	...
	(
		...
		SELECT
			...
			-- 2026-07-07 Effort-B: @INV entered basis (see header). Price drops × pd.FactorToBase ->
			...
			ROUND(pd.FinalPrice,2),                            -- Price: entered goods (no landed at insert)
			-- OLD: pd.BaseFinalQty,
