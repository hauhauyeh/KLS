-- RecalcQAV-v1 - LIVE baseline captured 2026-07-07 (pre-fix rollback reference).
-- Frozen restore point BEFORE the fix (base-unit RecentCost lookup + ValueTolerance-gated
-- normalize/absorb). Logic is byte-identical to the deployed proc; only the CREATE verb
-- (-> CREATE OR ALTER) and the ANSI/QUOTED_IDENTIFIER + GO wrappers are added so it is
-- directly re-deployable to roll back. Do not edit.
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE OR ALTER PROCEDURE [dbo].[RecalcQAV-v1] --EXEC [RecalcQAV] 419,'02/25/26'
    @ItemId INT,
    @BeginDate DATE,
