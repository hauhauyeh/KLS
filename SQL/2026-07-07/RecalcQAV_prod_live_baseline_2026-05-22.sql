    ...
    */
    ...
    BEGIN
        ...
        BEGIN
            ...
            SET @DiffInventoryValue = ROUND(@Qty * @LAvgCost, 6) * -1
        ...
        -- 695 = After Receiving
        -- 250 (Inventory Adj Opening) is retired and should no longer be treated
        ...
        BEGIN
            ...
            -- This avoids Qty*Price rounding drift that unbalances the balance sheet.
              ...
              -- Step 1: Set closing quantity
              SET @CloQty = @Qty

              -- Step 2: Read existing Amount directly (the exact TotalValue from old ledger)
              SELECT @Amount = ISNULL(Amount, 0)
              FROM TransactionJournalDetail
              WHERE TxDetailId = @MyTxDetailId

              -- Step 3: Calculate inventory value from preserved Amount
              SET @InventoryValue = @LInventoryValue + @Amount

              -- Step 4: Derive average cost from exact inventory value
              SET @AvgCost = CASE WHEN @CloQty != 0 THEN ROUND(@InventoryValue / @CloQty, 6) ELSE 0 END

              -- Step 5: Update CloQty, AvgCost, InventoryValue only - leave Amount/CrDeAmount untouched
              UPDATE TransactionJournalDetail SET
                  ClosingQty = @CloQty,
                  AverageCost = @AvgCost,
                  InventoryValue = @InventoryValue
            ...
            ----PAIRED ACCOUNT (@OBE) RECALC - don't touch (already correct from GeneralJournal_OBINV posting)
            --IF @Amount = 0
            --    SET @CrDeAmt = 0; --SET @DebitAmt = 0; SET @CreditAmt = 0;
            --ELSE
            --    SET @CrDeAmt = CASE WHEN @Amount > 0 THEN ABS(@Amount) ELSE -ABS(@Amount) END;
