...
--
-- 2026-07-07 (Effort-B): @INV rows written here are ENTERED basis (was base/converted). Both journal paths
--   (ChangeStatus 'I' insert + 'U' update):
--     Price            = @FinalPrice   -- drop old (@FinalPrice x @FactorToBase); goods price is entered.
--     BillQty          = @FinalQty     -- entered value (was @BaseFinalQty).
--     FactorToBase slot = @ReceiveQty  -- repurposed ratio -> entered physical qty (audit, no consumer).
--   Qty stays @BaseReceiveQty (base physical). Landed is NOT added here (added at billing by
--   Shipment_AllocationInventoryClear, called at the end). This SUPERSEDES the old "price threading deferred"
--   note -- price is no longer deferred. ChangeStatus=NULL SURVIVOR rows are NOT touched here; the anchor
--   (InventoryClear, Option B) normalizes them from PurchaseDetail at billing. Deploys as ONE batch with
--   InventoryClear + Purchase_Insert + VendorPayment_InsertPayNow.
