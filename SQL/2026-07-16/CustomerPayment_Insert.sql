...
GO
-- FIX_IDG_AMOUNT_SIGN 2026-07-16: @IDG (Sales Discount Given, debit-normal contra-income) discount
-- line must store [Amount] as the MAGNITUDE (+@Amount) -- the same value fed to Fn_Adjust_CrDeAmount
-- for [CrDeAmount] -- not @Amount*-1. Same family as the @ICREDIT Amount-sign fix. Single spot.
-- See baseline: KLS/SQL/2026-07-16/CustomerPayment_Insert_live_baseline.sql
...
BEGIN
    ...
    DECLARE @PrevUnapplied DECIMAL(18,2) = 0; -- 2026-06-24: locked-edit refund clamp source
    DECLARE @LockedRestoreCredit BIT = 0; -- 2026-06-24: locked plain-credit needs Phase 7 @ExtraAmount recompute
    ...
    BEGIN
        SELECT
            ...
            @PrevUnapplied = ISNULL(UnappliedAmount, 0), -- 2026-06-24: locked-edit refund clamp source
    ...
    IF @IsLockedEdit = 1
    BEGIN
        -- 2026-06-24: capture an explicit "plain credit -> Refund" switch BEFORE the
        -- wipe below clears @AsRefund / @ExtraAmount. Only set when the payment carries
        -- NO prior non-credit earmark (Income / Refund / CCFee) to preserve -- i.e. the
        -- leftover is already plain unapplied credit, so converting it to a refund
        -- reserve changes nothing the locked-edit immutability rule needs to protect.
        -- Consumed by the new ELSE IF branch lower in this block.
        DECLARE @LockedCreditToRefund BIT =
            CASE WHEN ISNULL(@NewExtraDisposition, '') = 'Refund'
                  AND @HasExplicitDispositionSwitch = 1
                  AND @PrevAsIncome = 0
                  AND @PrevRefund = 0
                  AND @PrevCCFee = 0
                  AND ISNULL(@SelectedExtraDispositionAmount, 0) > 0
                 THEN 1 ELSE 0 END;

        -- 2026-06-24: same detection for an explicit "plain credit -> Income" switch
        -- (symmetric to the refund detection above). Consumed by the new ELSE IF branch.
        DECLARE @LockedCreditToIncome BIT =
            CASE WHEN ISNULL(@NewExtraDisposition, '') = 'Income'
                  AND @HasExplicitDispositionSwitch = 1
                  AND @PrevAsIncome = 0
                  AND @PrevRefund = 0
                  AND @PrevCCFee = 0
        ...
        END
        -- 2026-06-24: NEW branch -- plain unapplied credit converting to Refund on a
        -- locked edit. Reached only when there is no prior Income/Refund/CCFee earmark,
        -- so the leftover is plain credit. Honor an explicit "As Refund" switch from the
        -- edit UI instead of forcing it back to credit. This is the duplicate-charge
        -- cleanup path (e.g. Jinya #99223): the bank refund was already done manually;
        -- recording it As Refund lands the row in CustomerPayment_GetRefundQueue and
        -- posts the AsRefund reclass (Phase 9D). @LockedCreditToRefund was captured at
        -- the top of this block, before the wipe cleared @AsRefund / @ExtraAmount.
        ELSE IF @LockedCreditToRefund = 1
        BEGIN
            -- 2026-06-24: guard the now-mutable locked path -- never reserve more than
            -- the payment's actual unapplied credit. Reserving less is a valid partial
            -- refund; reserving more would over-credit AR, so block it.
            IF @SelectedExtraDispositionAmount > @PrevUnapplied + 0.005
                THROW 50013, 'Locked-payment refund cannot exceed the payment''s unapplied credit.', 1;

            SET @AsRefund = 1;
            SET @ExtraAmount = @SelectedExtraDispositionAmount;
        END
        ELSE IF @LockedCreditToIncome = 1
        BEGIN
            -- 2026-06-24: plain unapplied credit -> Income on a locked edit (symmetric to
            -- the refund branch). Same guard: cannot recognize more income than the
            -- payment's unapplied credit. AsIncome posts to its own income line (Phase 9C)
            -- and does NOT reduce AR, so no extra journal-balance handling is needed here.
            IF @SelectedExtraDispositionAmount > @PrevUnapplied + 0.005
                THROW 50013, 'Locked-payment income cannot exceed the payment''s unapplied credit.', 1;
        ...
        ELSE
        BEGIN
            -- 2026-06-24: forced plain credit on a locked edit (no recognized disposition
            -- switch). Flag so Phase 7 recomputes @ExtraAmount from committed totals -- the
            -- override zeroed it, which dropped the unapplied credit from the AR control
            -- line and unbalanced the journal (see #92856 et al.).
            SET @AsCredit = 1;
            SET @LockedRestoreCredit = 1;
        END;
    ...
    -- 2026-06-24: a locked payment forced back to plain AsCredit had @ExtraAmount zeroed
    -- by the locked-edit override (Phase 1B), dropping the unapplied credit from the AR
    -- control line and unbalancing the journal (#92856 et al.). Recompute the leftover from
    -- COMMITTED totals so it is correct whether the payment is fully unapplied or was
    -- re-apportioned in this same locked edit. Gated to the locked-forced-credit path only;
    -- non-locked AsCredit keeps its frontend-supplied @ExtraAmount.
    IF @LockedRestoreCredit = 1
        SET @ExtraAmount = @PaymentAmount - @DirectDocumentApplied + @CreditMemoUsed - @DirectCCFeeApplied;
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUES
            (
                -- 2026-07-16 FIX_IDG_AMOUNT_SIGN: Amount = discount magnitude (+@Amount), the value
                -- fed to Fn_Adjust_CrDeAmount above. @IDG (debit-normal contra-income) increases by
                -- the discount, so store +@Amount; CrDeAmount already carries the -D debit side.
                -- OLD: @TxId, @AccountId, @PayeeId, @Amount*-1, @CrDeAmount
                @TxId, @AccountId, @PayeeId, @Amount, @CrDeAmount
            );
        END;
    ...
    -- Phase 9E. 2026-06-24: journal balance assertion (guard rail). A committed
    -- double-entry document must net to zero on CrDeAmount. This catches any journal-drop
    -- bug (e.g. the locked plain-credit @ExtraAmount=0 corruption that left #92856 et al.
    -- unbalanced) at save time and rolls the whole save back via the outer CATCH, instead
    -- of silently writing unbalanced books.
    DECLARE @JournalNet DECIMAL(18,2) =
        (SELECT ISNULL(SUM(CrDeAmount), 0) FROM dbo.TransactionJournalDetail WHERE TxId = @TxId);
    IF ABS(@JournalNet) > 0.005
        THROW 50014, 'Customer payment journal does not balance (CrDe net <> 0). Save rolled back.', 1;
