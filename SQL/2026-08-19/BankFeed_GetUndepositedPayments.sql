...
--
-- DATE_FILTER 2026-08-07: optional @StartDate/@EndDate narrow the grid to a payment-date
-- range (plan/bank-feed-deposit-date-filter.md). Inclusive bounds - PaymentDate is type
-- date, no time component. Each bound applies independently; both NULL = unfiltered
-- (unchanged live behaviour). Same predicate in the count and page passes so paging stays
-- consistent. A filter, never a gate - like @Search.
--
-- METHOD_FILTER 2026-08-19: optional @PaymentMethod narrows the grid to one payment method
-- (plan/bank-feed-deposit-method-filter.md). Exact match, not LIKE - PaymentMethod is a small
-- enumerated set (CHECK, ACH, E-CHECK, CASH, CREDIT CARD on live candidates). NULL =
-- unfiltered (unchanged live behaviour). Same predicate in the count and page passes so
-- paging stays consistent. A filter, never a gate - like @Search and the date bounds.
--
...
CREATE OR ALTER PROCEDURE [dbo].[BankFeed_GetUndepositedPayments]   -- EXEC dbo.BankFeed_GetUndepositedPayments @BankFeedTransactionId=1, @Search=NULL, @StartDate=NULL, @EndDate=NULL, @PaymentMethod=NULL, @Pageno=1, @Pagesize=25, @IsCount=0, @TotalCount=NULL
    @BankFeedTransactionId BIGINT,
    @Search                NVARCHAR(100) = NULL,
    @StartDate             DATE          = NULL,   -- DATE_FILTER 2026-08-07
    @EndDate               DATE          = NULL,   -- DATE_FILTER 2026-08-07
    @PaymentMethod         NVARCHAR(50)  = NULL,   -- METHOD_FILTER 2026-08-19
    @Pageno                INT           = 1,
    @Pagesize              INT           = 25,
    @IsCount               BIT           = 0,
...
BEGIN
    ...
    BEGIN
        ...
        WHERE cp.IsLocked = 0
          ...
          AND (@StartDate IS NULL OR cp.PaymentDate >= @StartDate)   -- DATE_FILTER 2026-08-07
          AND (@EndDate   IS NULL OR cp.PaymentDate <= @EndDate)     -- DATE_FILTER 2026-08-07
          AND (@PaymentMethod IS NULL OR cp.PaymentMethod = @PaymentMethod)   -- METHOD_FILTER 2026-08-19
          AND (@Search IS NULL
