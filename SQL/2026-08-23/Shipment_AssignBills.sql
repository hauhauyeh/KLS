...
-- (insert link -> reset completion -> provisional allocation), but is deliberately stricter
...
CREATE OR ALTER PROCEDURE [dbo].[Shipment_AssignBills] -- EXEC dbo.Shipment_AssignBills @ShipmentId = 1, @PurchaseIds = '1,2'
    @ShipmentId  INT,
...
BEGIN
    ...
    BEGIN TRY
        BEGIN TRAN;
            ...
            -- 2) Keep immediate allocation, but reset completion and defer final AP
            -- charge bill generation to explicit confirmation.
