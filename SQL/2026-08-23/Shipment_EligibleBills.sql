...
BEGIN
    ...
    DECLARE @TargetContainerNormalized NVARCHAR(100);

    SELECT @TargetContainerNormalized =
        NULLIF(REPLACE(REPLACE(UPPER(LTRIM(RTRIM(s.ContainerNo))), ' ', ''), '-', ''), '')
    FROM dbo.Shipment s
    ...
    SELECT TOP (100)
        ...
        CAST(CASE
            WHEN @TargetContainerNormalized IS NOT NULL
             AND NULLIF(REPLACE(REPLACE(UPPER(LTRIM(RTRIM(p.ContainerNumber))), ' ', ''), '-', ''), '') = @TargetContainerNormalized
            THEN 1 ELSE 0
    ...
    ORDER BY ContainerMatchRank ASC, p.PurchaseNumber DESC;
END
