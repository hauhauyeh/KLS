...
CREATE OR ALTER PROCEDURE [dbo].[Purchase_AssignedShipment] -- EXEC dbo.Purchase_AssignedShipment @PurchaseId = 1, @IsShipment = 0
    @PurchaseId INT,
    @IsShipment BIT
AS
BEGIN
    ...
    SELECT ROW_NUMBER() OVER(ORDER BY sp.ShipmentPurchaseId) AS Id,
        ...
        CAST(CASE WHEN EXISTS (
            SELECT 1 FROM dbo.Purchase bill
            WHERE bill.SourceShipmentId = s.ShipmentId
              AND ISNULL(bill.IsShipment, 0) = 1
              AND (ISNULL(bill.IsLocked, 0) = 1 OR ISNULL(bill.PaymentApplied, 0) > 0)
        ) THEN 1 ELSE 0 END AS BIT) AS IsGeneratedApLockedOrPaid
