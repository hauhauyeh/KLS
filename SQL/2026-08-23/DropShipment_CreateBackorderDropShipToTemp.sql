...
-- 2026-08-09 BACKORDER-DROPSHIP-ROOT-CHAIN: source SO can seed only when it is the latest row in its root chain.
-- 2026-08-23 BACKORDER-DROPSHIP-ZERO-SEED: seed all eligible source item lines; fully shipped/over-shipped lines start at qty 0.
...
BEGIN
    ...
    BEGIN
        SELECT
            ...
            CAST('Backorder drop-ship already exists after this source order' AS NVARCHAR(200)) AS [Message];
        RETURN;
    END
    ...
    SELECT
        ...
        CAST(
            CASE
                WHEN ISNULL(sd.OrdQty, 0) - ISNULL(sd.ShipQty, 0) > 0
                    THEN ISNULL(sd.OrdQty, 0) - ISNULL(sd.ShipQty, 0)
                ELSE 0
            END
            AS DECIMAL(18,2)
        ),
    ...
    BEGIN
        SELECT
            ...
            @FactorPO AS FactorPO,
            0 AS GeneratedLineCount,
            CAST('No source item row available for backorder' AS NVARCHAR(200)) AS [Message];
        RETURN;
    END
