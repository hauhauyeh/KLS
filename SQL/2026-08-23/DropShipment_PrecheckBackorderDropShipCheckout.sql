...
-- 2026-08-23 BACKORDER-DROPSHIP-FLEX-CART: source item rows may be zero qty; added item/account rows are allowed.
...
BEGIN
        ...
        WHERE ts.EmpId = @EmpId
          ...
          (
              ...
              (
                  ...
                  (
                      ts.LineType = 'I'
                      AND ts.ItemId IS NOT NULL
                      AND ts.AccountId IS NULL
                      AND ts.ItemUnitId IS NOT NULL
                      AND ISNULL(ts.OrdQty, 0) >= 0
                      AND ISNULL(ts.OrdQty, 0) = ISNULL(ts.ShipQty, 0)
                      AND ISNULL(ts.OrdQty, 0) = ISNULL(ts.BillQty, 0)
                  )
                  AND NOT
                  (
                      ts.LineType = 'A'
                      AND ts.AccountId IS NOT NULL
                      AND ts.ItemId IS NULL
                      AND ISNULL(ts.OrdQty, 0) > 0
                      AND ISNULL(ts.OrdQty, 0) = ISNULL(ts.ShipQty, 0)
                      AND ISNULL(ts.OrdQty, 0) = ISNULL(ts.BillQty, 0)
                  )
    ...
    BEGIN
        SELECT CAST(0 AS BIT) AS CanPost, CAST('Backorder drop-ship cart contains unsupported rows.' AS NVARCHAR(200)) AS [Message];
        RETURN;
    END
    ...
    (
        ...
        WHERE ts.EmpId = @EmpId
          ...
          AND
          (
              (ts.LineType = 'I' AND ISNULL(ts.OrdQty, 0) > 0)
              OR
              (ts.LineType = 'A' AND ROUND(ISNULL(ts.BillQty, 0) * ISNULL(ts.UnitPrice, 0), 2) <> 0)
          )
    )
    BEGIN
        SELECT CAST(0 AS BIT) AS CanPost, CAST('Backorder drop-ship must include a positive item quantity or non-zero account charge.' AS NVARCHAR(200)) AS [Message];
        RETURN;
    END
    ...
    WHERE ts.EmpId = @EmpId
      ...
      AND ts.IsStrike = 0
      AND ts.LineType = 'I'
      AND ts.DisplaySort IS NOT NULL
    GROUP BY
        ts.DisplaySort,
        ts.ItemId,
        ts.ItemUnitId;
    ...
    BEGIN
        SELECT CAST(0 AS BIT) AS CanPost, CAST('No source item row available for backorder.' AS NVARCHAR(200)) AS [Message];
        RETURN;
    END

    IF EXISTS
    (
        SELECT SourceLineId, ItemId, ItemUnitId FROM @EligibleRows
        EXCEPT
        SELECT SourceLineId, ItemId, ItemUnitId FROM @SeededTempRows
    )
    BEGIN
        SELECT CAST(0 AS BIT) AS CanPost, CAST('Backorder drop-ship cart is missing source item rows.' AS NVARCHAR(200)) AS [Message];
        RETURN;
    END
