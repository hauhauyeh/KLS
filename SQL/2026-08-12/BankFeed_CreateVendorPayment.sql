...
--
-- 2026-08-12 append-desc toggle (plan/plan-bankfeed-append-desc-notes-v1.md, Slice 1):
--   @AppendBankDesc BIT = 1 controls whether the bank row's Description reaches the
--   generated notes. ON (default, the old behavior for headers): document notes stay
--   'Bank feed: <description>' and charge DETAIL lines now gain '<user note> - <description>'
--   (description alone when the line note is blank) - detail lines never carried the
--   description before. OFF: header notes are NULL and detail lines carry only the user
--   note. The default keeps every existing caller byte-identical on headers.
--
-- 2026-08-12 resolve-only (plan/plan-bankfeed-resolve-only-v1.md, Slice 1):
--   A money-out row may now be ENTIRELY resolving lines - no bills at all (a standalone bank
--   fee, a service debit with no bill in the system). @LinesJson may be NULL/empty when
--   resolving lines are present; the one refusal is nothing at all (50107, reworded). The
--   bill guards all pass vacuously on an empty #Lines and the payment cursor runs zero
--   iterations, so the only real edits are: #Lines parsed conditionally, the combined
--   emptiness guard, ISNULL around SUM(ApplyAmount) in 50112/50136 (an empty-set SUM is
--   NULL, which made those comparisons UNKNOWN and silently skipped the THROW), and
--   @NewVendorPaymentId falling back to the first charge on a no-bill row.
--
...
CREATE OR ALTER PROCEDURE [dbo].[BankFeed_CreateVendorPayment]
    ...
    -- 2026-08-12 resolve-only: @LinesJson may be NULL when @ResolvingLinesJson has lines.
    -- 2026-08-12 append-desc toggle: pass @AppendBankDesc=0 to keep notes to user input only.
    ...
    -- 2026-08-12 append-desc toggle: 1 = write the bank description into the generated notes
    -- (headers as before, charge detail lines newly); 0 = keep all notes to what the user typed.
...
BEGIN
    ...
    ---------------------------------------------------------------------------------------
    -- 2026-08-12 resolve-only: an absent @LinesJson is now legal (the row may be entirely
    -- resolving lines), so #Lines is created explicitly and filled only when bills were
    -- sent. A non-empty but malformed @LinesJson still refuses here. The emptiness rule
    -- moves BELOW the resolving parse, where both sets are known. Old guards kept:
    --IF @LinesJson IS NULL OR ISJSON(@LinesJson) <> 1
    --    THROW 50107, 'No bills were selected for this payment.', 1;
    CREATE TABLE #Lines
    (
        PurchaseId     INT            NULL,
        ApplyAmount    DECIMAL(18,2)  NULL,
        DiscountAmount DECIMAL(18,2)  NULL
    );

    IF @LinesJson IS NOT NULL AND LTRIM(RTRIM(@LinesJson)) <> ''
    BEGIN
        IF ISJSON(@LinesJson) <> 1
            THROW 50107, 'The selected bills are not valid.', 1;

        INSERT INTO #Lines (PurchaseId, ApplyAmount, DiscountAmount)
        SELECT PurchaseId,
               ApplyAmount,
    ...
    -- 2026-08-12 resolve-only: the one emptiness rule for all three shapes - bills only,
    -- bills + resolving, resolving only. Nothing at all is the only refusal.
    IF NOT EXISTS (SELECT 1 FROM #Lines) AND NOT EXISTS (SELECT 1 FROM #Resolving)
        THROW 50107, 'Select at least one bill or enter a resolving line.', 1;
    ...
    BEGIN TRY
        ...
        -- 2026-08-12 resolve-only: ISNULL is load-bearing, not defensive. SUM over an empty
        -- #Lines is NULL, NULL <> x is UNKNOWN, and the THROW would be silently skipped -
        -- i.e. the balance invariant would not run at all on a no-bill row.
        IF ISNULL((SELECT SUM(ApplyAmount) FROM #Lines), 0) + @ResolvingTotal <> ABS(@BankAmount)
        ...
        -- 2026-08-12 resolve-only: same ISNULL rule as 50112 above.
        IF ABS(@BankAmount) - ISNULL((SELECT SUM(ApplyAmount) FROM #Lines), 0) < 0
        ...
        -----------------------------------------------------------------------------------
        -- 2026-08-12 append-desc toggle: NULL when the user opted out - CASE with no ELSE.
        --DECLARE @Notes NVARCHAR(255) = LEFT(N'Bank feed: ' + ISNULL(@Description, N''), 255);
        DECLARE @Notes NVARCHAR(255) =
            CASE WHEN @AppendBankDesc = 1
                 THEN LEFT(N'Bank feed: ' + ISNULL(@Description, N''), 255)
            END;
        ...
        BEGIN
            ...
            SELECT @EmpId, @ChargePayeeId, 0,
                   ...
                   1,
                   -- 2026-08-12 append-desc toggle: detail lines gain the description when
                   -- opted in - '<user note> - <description>', description alone when the
                   -- user note is blank. No 'Bank feed:' prefix here; it earns its place on
                   -- headers but doubles up as noise on every line (D3). Was: r.Notes
                   CASE WHEN @AppendBankDesc = 1
                        THEN LEFT(ISNULL(NULLIF(LTRIM(RTRIM(r.Notes)), N'') + N' - ', N'')
                                  + ISNULL(@Description, N''), 255)
                        ELSE r.Notes
                   END,
                   NULL
            FROM #Resolving AS r
            ...
            -- 2026-08-12 resolve-only: on a no-bill row the charge is the only document, so
            -- the success-signal OUTPUT falls back to the first charge. When the bill loop
            -- ran, it has already claimed this and the IF never fires.
            IF @NewVendorPaymentId IS NULL
                SET @NewVendorPaymentId = @ChargePaymentId;
