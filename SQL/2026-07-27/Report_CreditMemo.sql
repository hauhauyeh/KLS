-- 2026-07-27 SalesDocNumber Slice 12B: expose SalesDocNumber for credit memo report display.
CREATE OR ALTER PROCEDURE [dbo].[Report_CreditMemo]
    @StartDate DATE = NULL,
    @EndDate DATE = NULL,
    @SalesRep INT = NULL
AS
BEGIN
    ...
    WHERE s.ShipDate BETWEEN @StartDate AND @EndDate
      ...
      AND sd.ItemId NOT IN (SELECT ItemId FROM dbo.Item WHERE ItemCode LIKE '@%')
      AND sd.ExtTotal < 0
    GROUP BY s.SalesId, s.SalesNumber, s.SalesDocNumber, p.PayeeName, s.ShipDate, s.Instruction, eb.PayeeName
