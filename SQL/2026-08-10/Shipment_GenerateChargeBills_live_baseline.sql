...
CREATE   PROCEDURE dbo.Shipment_GenerateChargeBills -- EXEC dbo.Shipment_GenerateChargeBills @ShipmentId=61
    @ShipmentId INT
AS
BEGIN
    SET NOCOUNT ON;
        ...
        BEGIN
            ...
            IF NULLIF(LTRIM(RTRIM(ISNULL(@VendorDocNumber, ''))), '') IS NOT NULL
               AND EXISTS (
                ...
                THROW 50106, 'Shipment_GenerateChargeBills: Vendor DocNum already exists for this vendor.', 1;
...
NextBill:
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM dbo.ShipmentChargeBill scb
        JOIN dbo.ShipmentChargeBillLine scbl
          ON scbl.ShipmentChargeBillId = scb.ShipmentChargeBillId
        WHERE scb.ShipmentId = @ShipmentId
        GROUP BY scb.ShipmentChargeBillId, scb.PurchaseId
        HAVING SUM(scbl.ChargeAmount) > 0
           AND scb.PurchaseId IS NULL
    )
    BEGIN
        UPDATE dbo.Shipment
        SET Status = 'Closed',
