-- =============================================================================
-- Sales_PartialUpdate -- deploy (DROP + CREATE). Working file per SP workflow.
-- 2026-07-04: Phase B -- base qty derives from ItemUnitId -> ItemUnit (source of truth).
--   Two spots (same Fn_QtyToBase(qty, mult, factor) call, sourced by code shape):
--     (a) per-line WHILE loop -> scalar SUBQUERY lookup from ItemUnit by @ItemUnitId
--         (subquery form -> NULL on no-match; avoids the 'SELECT @x=col' retains-prior-row footgun).
--     (b) Section-9 set-based UPDATE -> INNER JOIN ItemUnit (item lines only; account lines
--         no ItemUnitId -> base qty untouched).
--   @FactorToBase (cart snapshot) is UNCHANGED -- still written to the FactorToBase snapshot columns.
--   Skip-check 2026-07-04: 4 ancient ItemUnitId-NULL item lines, 0 in last 90d.
-- Baseline: KLS/SQL/2026-07-04/Sales_PartialUpdate_live_baseline.sql
-- =============================================================================
SET ANSI_NULLS ON
GO
...
DROP PROCEDURE IF EXISTS [dbo].[Sales_PartialUpdate]
GO
...
BEGIN
	...
	BEGIN
        ...
        -- @FactorToBase (from the cart, above) is unchanged -- it still feeds the FactorToBase snapshot columns.
