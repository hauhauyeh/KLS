-- =============================================================================
-- Sales_Insert -- deploy (DROP + CREATE). Working file per SP workflow.
-- 2026-07-04: Phase B pilot -- thread rational-unit conversion in Section 9.
...
--   ItemUnit ratios are immutable, so the join is authoritative and the sd.FactorToBase
--   snapshot read is retired here. INNER JOIN = item lines only; account lines (LineType='A',
--   no ItemUnitId) have no unit -> base qty left NULL (they feed no @INV row).
--   Identity today (all MultipleToBase=1 -> qty/factor); enables combine-up (xN) forward.
--   SAME signature -- Fn_QtyToBase(qty, iu.MultipleToBase, iu.FactorToBase) + INNER JOIN
--   ItemUnit -- to be reused verbatim on the other sales-side Section-9 sites
--   (Sales_PartialUpdate, DropShipment_ConvertPOToBill, TempBombSales_Save).
-- Baseline: KLS/SQL/2026-07-04/Sales_Insert_live_baseline.sql
-- =============================================================================
SET ANSI_NULLS ON
GO
...
DROP PROCEDURE IF EXISTS [dbo].[Sales_Insert]
GO
