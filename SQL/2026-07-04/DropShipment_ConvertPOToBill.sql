-- =============================================================================
-- DropShipment_ConvertPOToBill -- deploy (DROP + CREATE). Working file per SP workflow.
-- 2026-07-04: Phase B (sales side) -- the SalesDetail Section-9 base-qty refresh now
--   derives from ItemUnitId -> ItemUnit (source of truth) via dbo.Fn_QtyToBase, INNER
--   JOIN ItemUnit (item lines only; account lines untouched). Same signature as
--   Sales_Insert / Sales_PartialUpdate. Identity today (all MultipleToBase=1).
--   OUT OF SCOPE (unchanged): the PurchaseDetail BaseReceiveQty/BaseFinalQty computes
--   (~L151-168, purchase side) and the sd.FactorToBase journal passthrough (~L409).
--   NOTE (pre-existing, not changed): the ShipQty=BillQty fill runs AFTER Section 9,
--   so BaseShipQty is computed from the pre-fill ShipQty -- unchanged by this edit.
-- Baseline: KLS/SQL/2026-07-04/DropShipment_ConvertPOToBill_live_baseline.sql
-- =============================================================================
SET ANSI_NULLS ON
GO
...
DROP PROCEDURE IF EXISTS [dbo].[DropShipment_ConvertPOToBill]
GO
