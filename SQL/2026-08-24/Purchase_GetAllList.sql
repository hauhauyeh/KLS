...
CREATE OR ALTER PROCEDURE [dbo].[Purchase_GetAllList] -- EXEC dbo.Purchase_GetAllList @Pageno = 1, @Pagesize = 50, @Search = NULL, @StartDate = NULL, @EndDate = NULL, @VendorId = NULL, @EmpId = NULL, @Filterby = NULL, @Id = NULL, @SortField = NULL, @SortOrder = NULL, @IsCount = 0, @DropShipSalesCustomerId = NULL, @TotalCount = NULL
    @Pageno INT,
    @Pagesize INT,
    @Search NVARCHAR(50),
    @StartDate DATE,
...
BEGIN
    ...
    DECLARE @SafeSortOrder NVARCHAR(4) = CASE WHEN UPPER(@SortOrder) = 'DESC' THEN 'DESC' ELSE 'ASC' END;
    ...
    BEGIN
        ...
        spx.ShipmentLinkCount AS AssignedShipmentCount,
        shipinfo.ShipmentContainerNos,
        shipinfo.ShipmentCarrierName,
        shipmentstate.ShipmentAreChargesComplete,
        shipmentstate.ShipmentChargesCompletedAt,
        shipmentstate.ShipmentChargesCompletedBy,
    ...
    SET @Qry += ' FROM Purchase AS p INNER JOIN Payee AS v ON p.PayeeId = v.PayeeId
        ...
        OUTER APPLY (
            SELECT
                s.AreChargesComplete AS ShipmentAreChargesComplete,
                s.ChargesCompletedAt AS ShipmentChargesCompletedAt,
                s.ChargesCompletedBy AS ShipmentChargesCompletedBy
            FROM dbo.Shipment s
            WHERE s.ShipmentId = COALESCE(p.SourceShipmentId, shipinfo.ShipmentId)
        ) shipmentstate
        OUTER APPLY (
    ...
    SET @OrderBy =
        CASE LOWER(ISNULL(@SortField, ''))
            WHEN 'purchasenumber' THEN 'p.PurchaseNumber'
            WHEN 'stageid' THEN 'p.StageId'
            WHEN 'arrivaldate' THEN 'p.ArrivalDate'
            WHEN 'vendordocnumber' THEN 'p.VendorDocNumber'
            WHEN 'payeeid' THEN 'v.PayeeName'
            WHEN 'vendortotal' THEN 'p.VendorTotal'
            WHEN 'purchasetotal' THEN 'p.PurchaseTotal'
            WHEN 'paymentstatusid' THEN 'psCalc.PaymentStatusId'
            ELSE NULL
        END;

    IF @OrderBy IS NOT NULL
        SET @Qry += ' ORDER BY ' + @OrderBy + ' ' + @SafeSortOrder + ', p.PurchaseNumber DESC';
    ELSE
    BEGIN
