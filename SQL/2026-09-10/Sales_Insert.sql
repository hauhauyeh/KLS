...
-- 2026-08-29 IsManualPrice pass-through (plan-reprice-open-orders-v1 slice 2) + IsPricePending flag after header insert. Baseline: Sales_Insert_live_baseline.sql
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            RAISERROR('Credit memo lines must have negative or zero totals.', 16, 1);
            RETURN;
        END
    ...
    BEGIN TRY
        ...
        -- 2026-08-29 Price-pending flag (plan-reprice-open-orders-v1 slice 2, D2=B):
        -- an order posted on the price-update day before the weekly cost apply ran is
        -- flagged so Sales_RepriceOpenOrders re-prices it after the apply. Prices are
        -- saved normally (not zeroed). Fn_IsPriceUpdatePending never throws (returns 0).
        IF @StageId = 0 AND @ShipDate > CAST(GETDATE() AS DATE) AND dbo.Fn_IsPriceUpdatePending() = 1
            UPDATE dbo.Sales SET IsPricePending = 1 WHERE SalesId = @SalesId;
        ...
        WHEN NOT MATCHED THEN
            ...
            (
                ...
                NULL, NULL,
                source.CartLineType, source.IsSystemManaged, source.DisplaySort, source.IsManualPrice
            )
        OUTPUT
            source.TempSalesId,
