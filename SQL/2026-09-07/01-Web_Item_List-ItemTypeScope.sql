-- =============================================================================
-- Web_Item_List -- replace the hardcoded ItemType filter with WEB_ITEM_TYPE_SCOPE.
-- 2026-09-07 (plan-web-noninventory-catalog-v2, Slice 1): a new client's catalog is
--   entirely NonInventory, so the hardcoded ItemType = 'Inventory' returned zero rows.
--   The setting selects the catalog scope per tenant: Inventory (default) | NonInventory
--   | All. The value is mapped to a LITERAL predicate and never concatenated into @Qry.
--   Unknown, blank, or missing values fall through to 'Inventory' = pre-2026-09-07
--   behavior, so every existing tenant is unaffected until its row says otherwise.
-- DEPLOY ORDER: deploy WITH 02-Web_Item_SearchInventory-ItemTypeScope.sql. That SP
--   carries the same hardcoded filter and feeds public portal search; deploying only
--   this one leaves search returning nothing for a NonInventory tenant.
-- Baseline: KLS/SQL/2026-09-07/Web_Item_List_live_baseline.sql
-- =============================================================================
SET ANSI_NULLS ON
GO
...
-- EXEC dbo.Web_Item_List @Pageno=1, @Pagesize=24, @PayeeId=1, @Search=NULL, @InStockOnly=0, @CategoryId=NULL, @IsWishList=0, @SortField=NULL, @IsCount=0, @TotalCount=NULL
...
BEGIN
    ...
    -- 2026-09-07: per-tenant catalog scope. A missing row leaves @ItemTypeScope NULL and
    -- a blank SettingValue leaves it '', so NULLIF + ISNULL collapse both to 'Inventory'.
    ...
    -- 2026-09-07: ItemType is no longer hardcoded; the rest of this block (Inactive and
    -- the hidden-category exclusion) is unchanged from the baseline.
    -- SET @Qry += ' AND i.ItemType = ''Inventory'' and i.Inactive=0
    SET @Qry += ' AND i.Inactive=0
    ...
    IF @ItemTypeScope = 'NonInventory'
        SET @Qry += ' AND i.ItemType = ''NonInventory'' '
    ELSE IF @ItemTypeScope <> 'All'
        SET @Qry += ' AND i.ItemType = ''Inventory'' '
    ...
    SELECT
        ...
        CASE
            WHEN img.Has900 = 1
            THEN CONCAT(''/Images/items/'', img.ItemId, ''/'', img.ImageIndex, ''-900.png'')
            WHEN img.Has300 = 1
    ...
    OUTER APPLY
    (
        SELECT TOP 1 im.ItemId, im.ImageIndex, im.Has300, im.Has900
        FROM ItemImage im
