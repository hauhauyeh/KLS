...
--
-- 2026-05-11 change scope (rev 3):
--   Add a locked-payment partial-rebuild branch so editing the apply of a
--   deposited (IsLocked=1) payment no longer wipes IsLocked and no longer
--   severs the bank-recon link by deleting the TransactionJournal header.
--
--   Locked-edit path:
--     - keep the CustomerPayment header row (IsLocked stays 1)
--     - keep the TransactionJournal header row (bank-recon TxId stays anchored)
--     - rebuild only the child rows (CustomerPaymentDetail + TransactionJournalDetail)
--     - immutability guard runs BEFORE Phase 1A and rejects any change to:
--         PayeeId, PaymentDate, PaymentMethod, PaymentAmount, PaymentType,
--         and (for Customer Refund) FromAccountId.
--       PaymentType is included because it controls Bad Debt / Credit Apply /
--       Customer Refund branching downstream and would otherwise let a
--       tampered request change the entire posting mode of a deposited payment.
--     - force disposition (AsCredit / AsIncome / AsRefund / AsCCFee / ExtraAmount)
--       and CCFee back to the prior saved state regardless of request input,
--       so a tampered request cannot change accounting on a deposited payment.
--       The C# service only guards the 4 immutable header fields, so the SP
--       must self-defend on these.
--     - on the final header refresh, only touch PaymentApplied / UnappliedAmount /
--       Notes / ReferenceId / UpdatedAt. The header AsIncome column is refreshed
--       naturally by the AsIncome posting block when the prior state was income-
--       based, and is left alone (matching its prior saved value) otherwise.
--
--   Unlocked edits and new inserts continue to use the original full-rebuild path.
-- =====================================================================
