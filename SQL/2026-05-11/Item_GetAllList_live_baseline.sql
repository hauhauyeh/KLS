...
(
 ...
 = NULL,
    ...
    -- Old @ShowDeleted BIT = 0 removed 2026-05-08 (forward-removal 
of D toggle).
    ...
    --                Default 0 keeps the page list
 active-only as before.
    @ShowInactive  BIT = 0
)
AS
BEGIN
...
AdjDate DATE,
    ...
    IF @IsCount = 1
        SET @Qry = 'SELECT @RCount = COUNT(i.ItemI
d)';
    ELSE
        SET @Qry = '
            SELECT
            ...
            i.M0,
                ...
                CASE
                    WHEN im.ImageId IS NOT NULL AND im.Has300 = 1
                        THEN ''/Images/items/'' + CAST(im.ItemId AS VARCHAR(10)) + ''/'' + CAS
T(im.ImageIndex AS VARCHAR(10)) + ''-300.png''
                    ELSE NULL
                END,
                i.CategoryId,
                ISNULL(v.RootNode, '''') AS FullCategoryPath,
                i.StorageId,
                s.DisplayName AS Sto
rageName,
    ...
    IF @StartDate IS NOT NULL
        SET @Qry += '
            AND i.ItemId IN (
                ...
                FROM Purchase p
                INNER JOIN Purc
haseDetail pd ON p.PurchaseId = pd.PurchaseId
...
CategoryId
        ...
        SET @Qry += ' AND i.CategoryId IN (' + @CatI
ds + ')';
    END;
    ...
    IF @IsCount = 1
    BEGIN
        EXEC sp_executesql @Qry, N'@RCount INT
 OUTPUT', @RCount = @TotalCount OUTPUT;
        RETURN;
    END;
    ...
    ELSE
        SET @Qry +
= ' ORDER BY i.Last3M DESC, ItemName';
