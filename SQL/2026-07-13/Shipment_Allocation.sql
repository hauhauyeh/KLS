...
CREATE OR ALTER PROCEDURE [dbo].[Shipment_Allocation] -- EXEC Shipment_Allocation @PurchaseId=12164,@AllocationType='BY_VOLUME',@RefreshVolume=0
...
BEGIN
    ...
    DECLARE @StageId INT;
    -- 2026-07-13 DROPSHIP-EXCLUDE: also read IsDropShip so a drop-ship target can early-return.
    ...
    -- 2026-07-13 DROPSHIP-EXCLUDE: a drop-ship bill links for reference only -- never landed cost.
    -- Clear any stale landed cost on its own lines and return before any allocation basis/write.
    -- (Drop-ship SIBLING exclusion when a NORMAL bill drives shipment-wide realloc is handled by the
    -- #ImpactedPurchases filter below.)
    IF @IsDropShip = 1
    BEGIN
    ...
    BEGIN TRY
        ...
        -- 2026-07-13 DROPSHIP-EXCLUDE: exclude drop-ship bills from the impacted set. #Lines and
        -- #InlineFreight (the allocation basis) both derive from #ImpactedPurchases, so this one
        -- filter removes drop-ship from the denominator, ShipmentAllocation writes, and LandedCost
        -- writes. Prior:
        --   SELECT DISTINCT sp.PurchaseId INTO #ImpactedPurchases
        --   FROM dbo.ShipmentPurchase sp JOIN #Shipments s ON s.ShipmentId = sp.ShipmentId;
