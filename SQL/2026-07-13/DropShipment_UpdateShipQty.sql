...
-- Drop-ship customer-receipt. Sets the purchase-side receive state on the drop-ship PO lines
-- and transitions the linked sales order to Transit (StageId=3). No journal entries.
...
CREATE OR ALTER PROCEDURE [dbo].[DropShipment_UpdateShipQty] -- EXEC [dbo].[DropShipment_UpdateShipQty] @PurchaseId=12983, @EmpId=1
    @PurchaseId INT,
    @EmpId INT
AS
BEGIN
    ...
    BEGIN TRY
        ...
        -- 2026-07-13: fail fast -- the linked SO must transition; 0 rows means a broken link.
        IF @@ROWCOUNT = 0
        BEGIN
